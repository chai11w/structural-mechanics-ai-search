...
from tiku_shared.multi_question import (  # noqa: E402
    ...
    effective_question_chapter as _effective_question_chapter,
    finalize_ordered_multi_crops as _finalize_ordered_multi_crops,
    is_probable_structure_block as _is_probable_structure_block,
    normalize_multi_questions as _normalize_multi_questions,
    normalize_question_key as _normalize_question_key,
    prepare_multi_diagram_crops as _prepare_multi_diagram_crops,
    qwen_filter_structure_blocks as _qwen_filter_structure_blocks,
...
def normalize_multi_questions(raw_questions: Any) -> list[dict[str, Any]]:
    return _normalize_multi_questions(raw_questions)


def is_probable_structure_block(box: tuple[int, int, int, int, int], image_shape: tuple[int, ...]) -> bool:
    return _is_probable_structure_block(box, image_shape)
...
def finalize_ordered_multi_crops(
    ...
    return _finalize_ordered_multi_crops(questions, selected_paths, output_dir)


def build_block_contact_sheet(block_paths: list[Path], output_path: Path) -> Path:
    return _build_block_contact_sheet(block_paths, output_path)


def qwen_filter_structure_blocks(block_paths: list[Path], output_dir: Path, api_key: str) -> list[Path]:
    return _qwen_filter_structure_blocks(block_paths, output_dir, api_key)
...
def prepare_multi_diagram_crops(
    ...
    return _prepare_multi_diagram_crops(image_path, questions, output_root)
...
def effective_question_chapter(question: dict[str, Any]) -> str | None:
    return _effective_question_chapter(question, CHAPTERS)
...
def normalize_question_key(value: object) -> str:
    return _normalize_question_key(value)


def chinese_question_number_to_int(text: str) -> int | None:
    return _chinese_question_number_to_int(text)
