...
不要提取刚度 EI/EA、尺寸、节点编号、支座反力或公式结果。章节只能根据明确题干/方法文字判断；没有明确线索则 unknown。chapter_evidence 只能用中文引号原样引用图中实际可见文字，不得填写根据结构图形推理出的理由。
不要输出解释、Markdown或代码块。"""
...
def has_explicit_chapter_evidence(chapter_hint: str, chapter_evidence: str) -> bool:
    ...
    if chapter_hint == "2静定结构":
        # A model-written description such as "this looks like a truss" is
        # not chapter evidence.  Chapter 2 is especially prone to this false
        # positive because a bare structure diagram can visually look static.
        # Require an exact, quoted fragment of visible problem text before the
        # automatic chapter path is allowed to continue.
        if not quoted_text:
            return False
        check_text = quoted_text
        compact_text = re.sub(r"\s+", "", check_text).replace("各杆", "")
        if "不静定" in check_text or "超静定" in check_text:
            return False
        if any(trigger in quoted_text for trigger in triggers):
            return True
