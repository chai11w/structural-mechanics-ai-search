...
CHAPTER_TRIGGER_WORDS = {
    "2静定结构": ["静定结构", "静定梁", "静定刚架", "静定钢架", "静定桁架", "结点法", "节点法", "截面法"],
...
- 对 4力法、5位移法、6力矩分配、7矩阵位移、8影响线 必须看到明确方法文字或题干要求；不能因为结构是连续梁、超静定结构、标注 EI/刚度、支座较多，就推测为某种方法。
- 如果图片里只有结构图、荷载、支座、尺寸、EI 等信息，没有明确方法名或题干说明，chapter_hint 必须为 unknown。
- 如果题干文字明确给出典型题型，即使没有直接写章节名，也可以判断章节；但不能只靠结构图形猜。
- 文字出现“力矩分配法”或明确要求力矩分配，chapter_hint 为 6力矩分配。
- 文字出现“矩阵位移法”或明确要求用矩阵位移法计算，chapter_hint 为 7矩阵位移。
- 文字出现“影响线”或明确要求作/求影响线，chapter_hint 为 8影响线。
...
- 文字明确要求用结点法、节点法或截面法计算桁架杆件内力，chapter_hint 为 2静定结构。
- 题干明确是桁架指定杆轴力、静定桁架/静定梁/静定钢架内力分析，可判断为 2静定结构。
...
必须注意赋值:
- 如果题干或图片说明给出符号赋值，例如 P=40kN、q=20kN/m、F1=40kN、M=20kN·m，raw 必须保留“符号=数值”但删除单位：分别输出 P=40、q=20、F1=40、M=20；不要只输出符号，也不要输出带单位的赋值。
...
- 只有看见真实外荷载的箭头、分布箭头或力偶弧箭头时才允许提取荷载；单独的 P、F、Fp、q、M、kN 文字、刚度/尺寸标注和公式都不是荷载，不能仅凭符号生成 loads。
...
赋值回填（仅 single 时必须执行）：先看完整题干和图上说明中的等式，再识别箭头。若题干给出 Fp=28kN，而箭头旁写 Fp 或 Fp/2，则每个对应集中力的 raw 都输出 Fp=28；不得只输出 Fp 或 Fp/2。一般地，箭头符号在题干有数值赋值时，每个对应荷载都输出“符号=数值”且去掉单位，例如 P=40kN 输出 P=40、q=20kN/m 输出 q=20。不要输出带单位的赋值。
每一个独立外荷载都必须输出一条记录；不要把相同标注的多个荷载合并。例如三个分别标为 Fp 的集中力，必须输出三条 {"type":"集中","raw":"Fp"}。
只有符号旁确实存在对应的外力箭头、分布箭头或力偶弧箭头时才提取；孤立的 Fp/P/q/M 文字不能算荷载，不能生成 loads。
不要提取刚度 EI/EA、尺寸、节点编号、支座反力或公式结果。visible_problem_text 只抄写实际可见的完整题干/任务文字，节点字母、荷载符号、尺寸和 EI 不算题干，不得根据结构长相编写；没有题干时必须留空且章节为 unknown。只有 visible_problem_text 非空时，才允许结合题干与结构图判断章节。chapter_evidence 只能用中文引号原样引用图中实际可见文字，不得填写根据结构图形推理出的理由。
不要输出解释、Markdown或代码块。"""
  ...
  "questions":[
    {
      ...
      "chapter_hint":"2静定结构|3静定结构位移|4力法|5位移法|6力矩分配|7矩阵位移|8影响线|unknown",
      "chapter_confidence":0到1之间的小数,
      "visible_problem_text":"原样抄写本题实际可见的题干文字；没有题干则留空",
      "chapter_evidence":"原样引用用于判断章节的可见题干文字，使用中文引号；没有则留空"
    }
  ]
}
...
- 优先根据题目文字、方法名、题干说明判断章节，不要只根据结构图形猜章节。
- visible_problem_text 只允许抄写对应小题实际存在的完整题干/任务文字；节点字母、荷载符号、尺寸和 EI 不算题干，也不得根据结构长相编写题干。
- visible_problem_text 为空时，该题 chapter_hint 必须为 unknown；只有确实看见题干文字时，才允许结合题干与结构图判断章节。
- 对 4力法、5位移法、6力矩分配、7矩阵位移、8影响线 必须看到明确方法文字或题干要求；不能因为结构是连续梁、超静定结构、标注 EI/刚度、支座较多，就推测为某种方法。
- 如果图片里只有结构图、荷载、支座、尺寸、EI 等信息，没有明确方法名或题干说明，chapter_hint 必须为 unknown。
- 如果题干文字明确给出典型题型，即使没有直接写章节名，也可以判断章节；但不能只靠结构图形猜。
- 文字出现“矩阵位移法”或明确要求用矩阵位移法计算，chapter_hint 为 7矩阵位移。
...
荷载类型必须根据图形形态判断，不要根据单位判断。图形是分布箭头时，即使单位误写成 kN·m 或 kN.m，也输出均布，不要额外输出弯矩。
raw 不要输出单位：5kN/m、5kN·m、5kN.m 都只输出 5；q=20kN/m 输出 q=20；M=20kN·m 输出 M=20；赋值符号也保留等号左侧，如 P=40kN 输出 P=40；纯符号表达保持原样。
...
def has_explicit_chapter_evidence(chapter_hint: str, chapter_evidence: str) -> bool:
    ...
    evidence_text = str(chapter_evidence or "")
    check_text = quoted_text or evidence_text
    compact_text = re.sub(r"\s+", "", check_text).replace("各杆", "")
    if chapter_hint == "2静定结构":
        # Production callers pass separately transcribed visible problem text
        # into this validator.  Punctuation is not evidence: Qwen can copy the
        # right sentence while omitting the requested Chinese quotation marks.
        check_text = evidence_text
        compact_text = re.sub(r"\s+", "", check_text).replace("各杆", "")
        if "不静定" in check_text or "超静定" in check_text:
            return False
        if any(trigger in check_text for trigger in triggers):
            return True
        if "桁架" in check_text and any(word in check_text for word in ("指定杆", "轴力", "内力")):
            return True
        return bool(
            "静定" in check_text
            and any(word in check_text for word in ("内力图", "弯矩图", "剪力图", "轴力图"))
        )
    if chapter_hint == "3静定结构位移":
        check_text = evidence_text
        compact_text = re.sub(r"\s+", "", check_text).replace("各杆", "")
        if any(word in check_text for word in ("力法", "位移法", "转角位移", "力矩分配", "弯矩分配", "矩阵位移", "影响线")):
            return False
        if any(trigger in check_text for trigger in triggers):
            return True
        displacement_words = ("位移", "转角")
        static_displacement_clues = ("静定", "桁架", "EA为常数", "EA常数", "EA均为常数", "单位荷载", "图乘")
        return any(word in check_text for word in displacement_words) and any(
...
def guard_chapter_prediction(
    chapter_hint: str,
    confidence: float,
    evidence: str,
    visible_problem_text: str | None = None,
) -> tuple[str, float, str]:
    if visible_problem_text is not None:
        visible_text = str(visible_problem_text or "").strip()
        if not visible_text:
            return CHAPTER_UNKNOWN, min(confidence, 0.49), "未识别到可见题干文字"
        evidence_to_check = visible_text
    else:
        # Keep the helper backward compatible for archived records/tests. New
        # image paths always pass visible_problem_text explicitly.
        evidence_to_check = evidence
        if chapter_hint == "2静定结构" and not quoted_evidence_segments(evidence):
            guarded = (
    ...
    if has_explicit_chapter_evidence(chapter_hint, evidence_to_check):
...
def normalize_layout_result(parsed: dict, image_size: tuple[int, int] | None = None) -> dict:
    ...
    for index, item in enumerate(questions, 1):
        ...
        visible_problem_text = str(item.get("visible_problem_text") or "").strip()
...
def normalize_load_item(item: dict) -> dict:
    ...
    raw = strip_units(clean_raw(item.get("raw", "")))
    aliases = {
        "集中力": "集中",
        "点荷载": "集中",
        "均布荷载": "均布",
...
def classify_images(args: argparse.Namespace) -> int:
    ...
    if args.image:
        ...
        if args.skip_existing_assigned_symbolic:
            existing = load_existing_excel_index(root)

            def should_skip_assigned_symbolic(path: Path) -> bool:
                rel = str(path.relative_to(root)).replace("\\", "/")
                old = existing.get(rel)
                if not old:
                    return False
                try:
                    loads = json.loads(str(old.get("old_loads") or "{}")).get("loads", [])
                except json.JSONDecodeError:
                    return False
                category, _details = classify_loads(loads)
                return category == "main_assigned_symbolic"

            images = [p for p in images if not should_skip_assigned_symbolic(p)]
    if args.shuffle:
...
def main() -> int:
    ...
    parser.add_argument(
        "--skip-existing-assigned-symbolic",
        action="store_true",
        help="skip existing Excel rows whose stored loads are assigned symbolic values such as P=40",
    )
