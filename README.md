...
- 保守自动章节：只有题图出现明确方法文字时才自动识别章节；证据不足时要求用户手动选择，并记录失败样本用于后续优化。
...
│   ├── chapter_judgment_log.py    # 自动章节失败样本日志
...
- 回复 `+` 进入新增题目入库流程。
- 在候选页回复 `-1/-2/-3` 可删除错误候选，删除前会二次确认并备份。

自动章节识别失败后，如果用户手动输入章节，样本会写入 `data/feishu_chapter_failure_log.jsonl`。这个日志只用于后续调优章节判断，不应混入 GUI/CLI 全量检索样本。
