...
> C 端复杂题图 A1/A2/A3 的完整开发规范见 [`docs/8890_complex_image_agent_plan.md`](docs/8890_complex_image_agent_plan.md)。`8891` 保留权威分流 MVP；`8896` 提供可用的 A3 人工裁剪 MVP，固定流程为“整页结构化理解 -> 一次选择一道 -> 人工裁剪 -> 图文校验 -> A2 -> 返回剩余题目”。`8892` 只保留 Paddle/自动裁剪 V2 的离线试验，第一版不开发自由 Planner、LangGraph 或 DeepSeek Harness。
...
8896 是与 8891、现有飞书服务完全隔离的 A3 人工裁剪 MVP。它先用整页理解建立公共题干、题目单元和结构图绑定；A3 只提取题号和题干，对外荷载只做“存在且完整”的门禁，不抄录荷载、尺寸、节点或杆件标注。多题页展示题目按钮并一次只选一道，单题页直接进入裁剪。用户在原图上框选单个完整结构图后，服务端校验题图归属、结构、支座和外荷载完整性；未通过时根据模型逐项检查返回一句简短的结果和主因，通过后才由 A2 从裁剪图详细识别荷载，并结合代码生成的 `a2_context_text` 判断章节。该上下文按“父题标题 + 公共题干 + 子题标题”传递，确保组级方法名称也能进入每一道题的 A2 流程；候选和答案会显示所对应的原题标签。A3 始终保留整页题目目录并作为外层会话控制器；A2 进行中可通过“换题重新搜”切换原图中的题目，当前题重复选择不会清除已有候选；裸题号同时可能指原题和候选时先澄清，明确说“图片第 2 题”或“候选 2”则直接进入对应路径。答案返回后，已完成题目会被标记并回到剩余题目列表。MVP 不调用 Paddle，也不做系统自动裁剪：

```powershell
python -B scripts/run_tiku_agent_8896.py
```

默认访问 `http://127.0.0.1:8896`，独立 Cookie 为 `tiku_agent_8896_session`，A3/A2 会话、裁剪图、模型费用和反馈数据都位于 `.tmp_tiku_agent_a3_mvp_8896/`。裁剪页沿用“力答”主线 UI，支持桌面和手机、100%–300% 缩放、裁剪示例、重新选题、草稿恢复和手机返回键。模型无法可靠判断章节时仍进入 A2 的章节追问，不根据结构外形猜章节。
