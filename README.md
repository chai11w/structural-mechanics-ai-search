...
默认访问 `http://127.0.0.1:8896`，独立 Cookie 为 `tiku_agent_8896_session`，分流、A3/A2 会话、裁剪图、模型费用和反馈数据都位于 `.tmp_tiku_agent_a3_mvp_8896/`。上传新题或切换子题只重置当前检索状态，已经显示的题图、裁剪图、候选和答案继续保留到会话过期；新建对话或最后一次操作 2 小时后才统一清理。裁剪页沿用“力答”主线 UI，支持桌面和手机、100%–300% 缩放、裁剪示例、重新选题、草稿恢复和手机返回键。模型无法可靠判断章节时仍进入 A2 的章节追问，不根据结构外形猜章节。`--disable-triage` 只用于本地诊断固定 A3 路径，默认不开启。
