...
打开 `http://127.0.0.1:8790` 后可发题图或直接文字对话。左侧栏、顶部栏和底部输入框保持固定，只有中间消息区滚动。会话、上传题图、裁图和答案输出均位于 `.tmp_tiku_agent/`；上传原图会随当前会话历史恢复，刷新或 Demo 重启后仍可显示，最后一次交互后 2 小时统一过期。该入口会记录不含用户原话、图片路径或模型原文的结构化任务日志。
