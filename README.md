...
> C 端复杂题图 A1/A2/A3 的完整开发规范见 [`docs/8890_complex_image_agent_plan.md`](docs/8890_complex_image_agent_plan.md)。`8891` 保留权威分流 MVP；`8896` 已接成隔离的完整 A1/A2/A3 流程：A1 停止，A2 直接进入原检索，A3 进入“整页结构化理解 -> 一次选择一道 -> 人工裁剪 -> 图文校验 -> A2 -> 返回剩余题目”。`8892` 只保留 Paddle/自动裁剪 V2 的离线试验，第一版不开发自由 Planner、LangGraph 或 DeepSeek Harness。
...
Agent 现在只有 Intent V2，不再提供 V1 运行开关。线上 8790 由隐藏进程启动，并使用独立生产目录 `.tmp_tiku_agent_v2_prod_8790/`。8790 默认启用状态感知安全回答 V0：寒暄、致谢等普通安全对话会收到由当前业务阶段生成的脱敏摘要，模型可以自然组织语言，但不能执行操作、编造候选或答案；“支持哪些章节”这类必须准确的产品事实则从代码中的 `CHAPTERS` 生成不带教材章号的知识范围固定答案，其中矩阵位移和影响线明确限制为只支持含具体外荷载的题目；该固定事实由 8790、8793、8794 Agent 入口共同复用，不调用意图模型、回答模型或业务工具，也不修改会话状态；业务指令仍直接进入各自原状态机。8788 飞书保持纯搜题工作流，不接入寒暄或固定事实问答。8790 和 8794 默认同时运行并行智谱外荷载筛查：候选先提交时直接返回，筛查先明确判断 `no` 时切断尚未提交候选的原流程，章节追问或空结果最多等待 15 秒；筛查失败或超时保留原结果。8790 可用 `--disable-external-load-screen` 临时回退，长期看门狗回退可传 `-DisableExternalLoadScreen`。模型回答还会经过状态一致性、执行声明、内部字段和业务状态未变化校验，不合格时按当前阶段使用固定兜底。业务工具使用 `SUCCESS / NO_MATCH / NEEDS_INPUT / PARTIAL / TOOL_ERROR` 五态结果，固定状态机据此决定继续、澄清、回退或失败；`PARTIAL` 有可用结果时会在回复中显示降级提示，不再伪装成完整成功。视觉复筛会校正带EXIF旋转标记的输入图，但不修改题库源文件；普通图片仍发送原始字节。
...
8896 与 8891、现有飞书服务完全隔离，但复用 8891 的权威 A1/A2/A3 预检。A1 停止并说明原因；A2 带“已确认单题”标记直接进入原精确识别与检索，不重复执行旧多题判断；只有 A3 才进入人工裁剪。A3 先用整页理解建立公共题干、题目单元和结构图绑定，只提取题号和题干，对外荷载只做“存在且完整”的门禁，不抄录荷载、尺寸、节点或杆件标注。多题页展示题目按钮并一次只选一道，选中后在原图上框选单个完整结构图；服务端校验题图归属、结构、支座和外荷载完整性，未通过时只在裁剪区显示一句结果和主因，通过后先把裁剪图作为用户消息写入对话，再由 A2 详细识别荷载并结合“父题标题 + 公共题干 + 子题标题”判断章节。候选和答案显示原题标签；A2 进行中可通过“换题重新搜”切换原图题目，裸题号有歧义时要求明确说“图片第 2 题”或“候选 2”。MVP 不调用 Paddle，也不做系统自动裁剪：

```powershell
python -B scripts/run_tiku_agent_8896.py
```

默认访问 `http://127.0.0.1:8896`，独立 Cookie 为 `tiku_agent_8896_session`，分流、A3/A2 会话、裁剪图、模型费用和反馈数据都位于 `.tmp_tiku_agent_a3_mvp_8896/`。上传新题或切换子题只重置当前检索状态，已经显示的题图、裁剪图、候选和答案继续保留到会话过期；新建对话或最后一次操作 2 小时后才统一清理。裁剪页沿用“力答”主线 UI，支持桌面和手机、裁剪示例、重新选题、草稿恢复和手机返回键；原图始终适配裁剪区，不提供缩放控件。8896 的 A2 与 A3→A2 入口共用严格章节三态：七类支持题固定到对应题库，动力学等明确超范围题停止，公式或证据不足才进入章节询问；非中文题干要求上传含中文版本。`WAIT_CHAPTER` 中先用代码解析题型，模糊自然表达才交给 Intent V2，模型结果仍须通过共享目录校验；纯寒暄保留当前题，只有系统已提供兜底且用户明确同意时才全局搜索。8896 的 A2 视觉复筛使用独立的 8 路首轮、12 秒超时、最多 8 个低并发重试；共享 8790/8788/CLI 默认策略不变。该开关不启用在 8790、8794、8788 或 CLI。`--disable-triage` 只用于本地诊断固定 A3 路径，默认不开启。
