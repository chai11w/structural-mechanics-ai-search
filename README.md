...
8896 是与 8891、现有飞书服务完全隔离的 A3 人工裁剪 MVP。它先用整页理解建立公共题干、题目单元和结构图绑定；A3 只提取题号和题干，对外荷载只做“存在且完整”的门禁，不抄录荷载、尺寸、节点或杆件标注。多题页展示题目按钮并一次只选一道，单题页直接进入裁剪。用户在原图上框选单个完整结构图后，服务端校验题图归属、结构、支座和外荷载完整性；通过后才由 A2 从裁剪图详细识别荷载，并结合代码生成的 `a2_context_text` 判断章节。答案返回后，已完成题目会被标记并回到剩余题目列表。MVP 不调用 Paddle，也不做系统自动裁剪：
