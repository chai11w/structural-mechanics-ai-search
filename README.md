...
8790/8794 网页中的服务端、网络、上传、认证、会话和额度失败都会生成可点赞/点踩的助手消息；登录或会话失效、图片不合格等可恢复错误会给出“重新登录”“重新上传题图”或“新对话”按钮，额度提示不会再被通用 5xx 文案覆盖。断网、健康检查失败或会话恢复失败会显示单次连接错误卡并提供“重新连接”，同一故障期间不重复刷屏；媒体失效、浏览器历史无法读取或保存也会说明影响。反馈提交失败保留弹窗内容并允许重新提交。等待中的临时消息不能评价。五态 `PARTIAL` 有可用结果时会在回复中显示降级提示，不再伪装成完整成功。此处仅改变 Agent 网页和共用状态机；8788 飞书、参数式 CLI 不增加网页交互，8793 继续保留冻结快照。
