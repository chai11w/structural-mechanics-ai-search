...
正式切换使用专用脚本。默认调用只做健康、路径、ID/哈希/状态一致性和当前看门狗唯一性预检，不会停止进程；维护窗口确认后才添加 `-Apply`。状态检查用于防止后台测试过的邀请码状态意外覆盖旧线上状态。脚本会再次备份、精确停止已验证的看门狗与 8790 子进程、启动控制库模式并验证临时邀请码登录和动态停用；任一步失败都会恢复旧哈希配置模式：
