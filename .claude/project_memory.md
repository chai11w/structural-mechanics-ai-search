...
- v5 固定50题只读对照已完成（梁15/钢架20/桁架15，不含拱）：三方完全一致39题；对人工基准 V4=39/50、V5=40/50。V5 单题平均4.100s、P50=3.513s、P95=4.793s，另有一次27.825s长尾。当前结果只证明严格转录小幅优于v4，尚未证明桁架计算高度能力。
...
- 多段尺寸求和曾是共同短板；v4 已修复已知错例，v5 在固定50张真实题图上完成对照并修正一题“只有单轴尺寸却输出尺寸”的错误，但相对人工仅由39/50升至40/50。11个剩余差异里，多数是无完整显式标注却存在历史人工归一真值；不能把 `unknown` 自动算作模型错误或正确，需先统一业务口径。
...
1. 审阅 `.tmp_structure_dimension_eval/v5_comparison_50_20260809/dimension_v4_v5_manual_disagreements.html` 的11个差异项，先确认“无显式完整尺寸但历史人工给 L×L/2L×L”是否仍属于目标口径。
2. 为桁架增加受限几何计算方案（只允许明确边长/分段可推出的高度，无法唯一确定仍 unknown），建立专项小样本后再跑同一固定50题；重点比较 #36/#37 及计算题，不直接改历史数据库。
