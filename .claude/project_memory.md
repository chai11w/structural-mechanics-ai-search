...
- **M1–M6 实现全部完成**，roadmap Stage 4（状态感知安全回答）核心逻辑闭环。**注意**：实现完成 ≠ 验收完成——Codex 明确要求再加一轮真实交互测试（见 In Progress）。
- 分支最新提交含 Codex 的额外工作（收紧安全回答校验、候选重选支持、全局搜索用最终分等），动手前先 `git log --oneline -10`。
- 8794 服务当前**正在运行**：`127.0.0.1:8794`（后台 `python -B scripts/run_tiku_agent_8794.py --port 8794`），默认启用安全回答。浏览器 `http://127.0.0.1:8794` 可试。
...
- **下一轮：真实交互测试**（Codex 评审后明确要求，重点不是继续堆规则）：
  1. **真实口语测试**——用户实际会怎么说（口语化/方言/省略/错字），不是 fixture 里的书面例句。
  2. **连续点击测试**——多轮连续操作（发图→选章节→选候选→看答案→再发图）的状态流转与安全回答表现。
  3. **跨页面状态测试**——浏览器多页面/多会话下状态隔离与安全回答一致性。
  4. **评审链路同步**——handoff/评审文档要同步反映这轮新增测试，别只更新代码。
- 流程教训（Codex）：**不要在单元测试通过后就判断"全部完成"**；安全回答这类对真实口语敏感的功能，必须用真实用户话说一遍、点一遍才算验收。
- 评审通过后再提升 8790。

## Known Risks / Decisions

- **架构边界（Codex 明确认可，不要破坏）**：模型只负责组织安全回答措辞；状态与权限留给代码（白名单 context + 权限矩阵 + 输出契约校验）。模型永远拿不到业务状态，只能收到代码派生的脱敏摘要。
...
2. **下一步不是继续堆规则，是做真实交互测试**（真实口语/连续点击/跨页面），重点验证用户实际会怎么说、怎么点，并让评审链路（handoff）同步。
3. 8794 已在跑，可连真实模型验证；改完记得补 handoff + 提交推送（用户惯例）。
