...
- 当前工作线：8790 结构尺寸低成本预筛优化实验，已选 10 张结构类型不同的题图（缺拱结构）、尺寸归一规则已确认；不修改 8790 生产，不硬截断 22 候选。
...
- 8790 结构尺寸预筛实验（8794自主化暂缓）：10 张样本已选定并核验存在，尺寸归一规则已确认，实验脚本与千问识别未运行，对比文档未产出。
...
- 结构尺寸低成本预筛尚未实验验证，也未接入 8790 候选流程。
...
- 当前会话模型无法看图（Read 图片返回 Unsupported Image）且视觉 MCP 被禁用/拒绝；用户要求的“Claude侧识别结果”在无视觉模型会话中无法产出，需支持视觉的会话或由人工对照题图填写。
...
1. 新建独立实验脚本（复用 classify_question_bank.py 的图片编码、重试和 usage 记录模式），自定义只识别结构类型与整体外包络尺寸的 Prompt，结果写入忽略目录；不污染生产分类 Prompt。
2. 在 10 张选定图片上以 `qwen3.7-plus`（key 走 DASHSCOPE_API_KEY）运行识别，产出对比文档：每图结构类型、结构外包络长/宽、归一 size_key，并排千问结果与“我的识别”栏。
3. “我的识别”栏当前受阻（本会话模型无法看图、视觉 MCP 禁用），需在支持视觉的会话由 Claude 填写或人工对照题图填写；实验验证通过前不修改 8790 生产、不硬截断 22 候选。
