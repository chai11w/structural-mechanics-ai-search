...
- 更新前live字母库的179道钢架中有3道旧值含0（`3L×0`、`6L×0`、`4L×0`），46道桁架无0。三题已按已知钢架用qwen3.7-plus V5.2只读重跑，均保留正确水平单轴（3L/6L/4L）、竖向null、完整long_width unknown，且无尺寸链冲突；原因图文报告位于 `.tmp_structure_dimension_eval/v52_zero_frame_3/current_bank_vs_qwen37_v52_zero_frames.html`。
- 单轴存储最终采用旋转不变的简化方案：现有 `长×宽` 只存两边都明确的完整尺寸，新增唯一一列 `单边尺寸`，不区分横/竖；单边查询以后匹配候选的 `单边尺寸` 或完整 `长×宽` 任一边。已备份到 `F:\cc\_backups\7-题库检索\2026-08-09\before_single_side_dimension_20260809_130425` 后为6个live字母库Excel统一增加该列，并写入7道人工裁决：`49.jpg` 改完整 `3L×L`；闭口 `15.jpg/7.jpg` 分别写单边 `6L/4L`；此前确认的4道单轴题写 `L/2L/2L/L`，同时清空其错误完整尺寸。live重读280行、7个目标均通过，钢架/桁架完整尺寸中已无0。此前生成的横向/竖向/状态三列方案仅为临时预览，从未写入live。
- 拱不参与尺寸层；已备份到 `F:\cc\_backups\7-题库检索\2026-08-09\before_arch_dimension_cleanup_20260809_133949` 后清空字母库 `2静定结构/4拱/1内力/题目/8.jpg` 遗留的 `16×4`，相邻行重读与渲染检查正常。
- artifact-tool 导出的合法xlsx可能省略工作表dimension提示，openpyxl `read_only=True` 会得到 `max_row=None`；全库回填manifest和差异报告读取器均已改用不保存的普通读取模式。新增单边列后live manifest仍为280行，CLI帮助正常，尺寸相关44项测试通过。
...
- 尺寸层已接入 CLI、8788 MultiAgentCoordinator 与 8790 Agent 工具的共享链路，但保持实验开关默认关闭。仅 `route=symbolic`、上游已知且参与尺寸的结构类型、荷载粗筛100%候选严格 `>20` 时调用一次 qwen3.7-plus V5.2，不重复识别结构类型。完整对完整不一致才硬删；单边只做同值稳定提前，不同/缺失保留；模型异常、未校验、冲突、全删空均回退原候选。查询尺寸独立缓存。
...
- 尺寸层的纯逻辑与入口回归已验证：20条不触发，21条只调一次查询图尺寸，单边不误删，模型失败保留原候选，同图+模型+Prompt版本+结构类型的第二次尺寸识别命中独立缓存，全项目单测通过。尚未打开生产开关，也尚未用真实>20候选题跑入口对照。
...
1. 用一道真实产生 `>20` 个字母库满分候选的题开启实验尺寸层对照，检查候选变化、单次新增延迟和缓存命中延迟，再决定是否打开8790/8788生产配置。
2. 为桁架增加受限几何计算方案（只允许明确边长/分段可唯一推出的高度，无法唯一确定仍 unknown），建立专项小样本后再跑同一固定50题，不直接改历史数据库。
3. 真实入口对照确认无错删后，再决定是否打开8790/8788的生产配置；实验通过前不打开默认值。
