...
function feedbackSummaryRows(items, { manageable = false } = {}) {
  const rows = items.map((item) => {
    return `<tr><td class="col-number"><span class="cell-main feedback-number">${escapeHtml(item.feedback_number)}</span></td><td class="col-user"><div class="feedback-topic"><span class="feedback-preview">${item.preview_image ? `<img src="${attr(item.preview_image)}" alt="题目缩略图">` : icon('image')}</span><span class="feedback-copy"><span class="cell-main">${escapeHtml(feedbackUserLabel(item))}</span><span class="cell-sub">${formatDateTime(item.created_at)}</span></span></div></td><td class="col-rating">${statusBadge(item.rating, ratingText(item.rating))}</td><td class="col-scope">${feedbackScopeBadge(item.feedback_scope)}</td><td class="col-reason"><div class="tag-list">${item.tags.map((tag) => `<span class="tag">${escapeHtml(TAG_LABELS[tag] || tag)}</span>`).join('') || '<span class="cell-sub">未选择原因</span>'}</div></td><td class="col-detail"><div class="feedback-detail-text">${escapeHtml(item.detail || item.preview_text || '未填写补充说明')}</div></td><td class="numeric col-cost">¥${escapeHtml(item.cost?.estimated_cost_cny || '0.00')}</td><td class="col-status">${item.archived_at ? statusBadge('archived') : statusBadge(item.review_status)}</td><td class="col-actions"><div class="cell-actions ${item.archived_at ? 'archived-actions' : ''}">${feedbackControls(item, manageable)}</div></td></tr>`;
  }).join('');
  return rows || '<tr><td class="empty-row" colspan="9">暂无反馈</td></tr>';
}

function feedbackSummaryCards(items, { manageable = false } = {}) {
  const cards = items.map((item) => `<article class="feedback-card"><header class="feedback-card-head"><div class="feedback-card-badges">${feedbackScopeBadge(item.feedback_scope)}${statusBadge(item.rating, ratingText(item.rating))}</div>${item.archived_at ? statusBadge('archived') : statusBadge(item.review_status)}</header><span class="feedback-number feedback-card-number">${escapeHtml(item.feedback_number)}</span><div class="feedback-card-user"><span class="feedback-preview">${item.preview_image ? `<img src="${attr(item.preview_image)}" alt="题目缩略图">` : icon('image')}</span><span class="feedback-copy"><span class="cell-main">${escapeHtml(feedbackUserLabel(item))}</span><span class="cell-sub">${formatDateTime(item.created_at)}</span></span></div><div class="feedback-card-copy"><div class="tag-list">${item.tags.map((tag) => `<span class="tag">${escapeHtml(TAG_LABELS[tag] || tag)}</span>`).join('') || '<span class="cell-sub">未选择原因</span>'}</div><p>${escapeHtml(item.detail || item.preview_text || '未填写补充说明')}</p></div><footer class="feedback-card-foot"><span class="feedback-card-cost"><small>关联费用</small><strong>¥${escapeHtml(item.cost?.estimated_cost_cny || '0.00')}</strong></span><div class="cell-actions ${item.archived_at ? 'archived-actions' : ''}">${feedbackControls(item, manageable)}</div></footer></article>`).join('');
  return cards || '<div class="feedback-card-empty">暂无反馈</div>';
}

async function renderOverview() {
  const body = mountShell('overview', '今日概览', '了解今天发起的页面、实际检索题目、估算费用和待处理反馈。');
  try {
    const data = await api('/api/admin/overview');
    body.innerHTML = `<section class="metrics" aria-label="今日运营指标"><div class="metric"><span class="metric-label">${icon('activity')}今日检索</span><div class="metric-value">${usageCounts(data, 'metric-usage-counts')}</div><span class="metric-note">发起页面 / 实际检索题目</span></div><div class="metric"><span class="metric-label">${icon('wallet')}估算总费用</span><strong class="metric-value">¥${escapeHtml(data.today_cost_cny)}</strong><span class="metric-note">全站剩余 ¥${escapeHtml(data.global_remaining_cny)}</span></div><div class="metric"><span class="metric-label">${icon('users')}启用用户</span><strong class="metric-value">${Number(data.active_invites)}</strong><span class="metric-note">当前可登录使用</span></div><div class="metric"><span class="metric-label">${icon('down')}待处理点踩</span><strong class="metric-value">${Number(data.pending_negative_feedback)}</strong><span class="metric-note">需要复盘的反馈</span></div></section><section class="section"><div class="section-head"><div class="section-title"><h2>用户今日使用</h2><p>“页”是发起上传，“题”是实际进入题库检索；费用为模型估算。</p></div><a class="text-link" href="/invitations">管理邀请码 ${icon('next')}</a></div><div class="table-tool"><div class="table-scroll"><table class="data-table"><thead><tr><th>用户</th><th>状态</th><th class="numeric">今日检索</th><th class="numeric">今日费用</th><th class="numeric">剩余额度</th><th>最后活动</th></tr></thead><tbody>${inviteUsageRows(data.invites, 10)}</tbody></table></div></div></section><section class="section"><div class="section-head"><div class="section-title"><h2>最近反馈</h2><p>反馈范围跟随被评价的回复，不会把整页识别和单题检索混在一起。</p></div><a class="text-link" href="/feedback">查看全部 ${icon('next')}</a></div><div class="table-tool"><div class="table-scroll"><table class="data-table feedback-table"><thead><tr><th class="col-number">反馈编号</th><th class="col-user">用户</th><th class="col-rating">评价</th><th class="col-scope">范围</th><th class="col-reason">原因</th><th class="col-detail">说明</th><th class="numeric col-cost">关联费用</th><th class="col-status">状态</th><th class="col-actions"></th></tr></thead><tbody>${feedbackSummaryRows(data.recent_feedback)}</tbody></table></div></div></section>`;
    bindFeedbackLinks();
...
function modelSummaryHtml(models) {
  if (!models?.length) return '<p class="cell-sub">这条历史记录没有可用的逐模型明细。</p>';
  return `<div class="model-cost-list">${models.map((item) => `<div class="model-cost-item"><div><strong>${escapeHtml(item.model)}</strong><span>${escapeHtml(item.provider)} · ${Number(item.call_count)} 次调用</span></div><div class="model-cost-values"><strong>¥${formatCostMicros(item.estimated_cost_micros)}</strong><span>${formatTokenCount(item.input_tokens)} 入 / ${formatTokenCount(item.output_tokens)} 出</span></div></div>`).join('')}</div>`;
}

function flowHtml(cost) {
  if (!cost?.flow?.length) return '<div class="case-expired">这条历史反馈尚未关联到费用流程记录。</div>';
  return `<div class="flow-list">${cost.flow.map((stage) => {
    const calls = stage.calls || [];
    const callRows = calls.map((call) => `<tr><td><span class="cell-main">${escapeHtml(call.label)}</span><span class="cell-sub">${escapeHtml(call.model)}</span></td><td>${call.status === 'success' ? statusBadge('resolved', '成功') : statusBadge('pending', '失败')}</td><td class="numeric">${formatTokenCount(call.input_tokens)} / ${formatTokenCount(call.output_tokens)}</td><td class="numeric">${formatDuration(call.latency_ms)}</td><td class="numeric">¥${formatCostMicros(call.estimated_cost_micros)}${call.pricing_status === 'repriced' ? '<span class="price-note">按现价补算</span>' : ''}</td></tr>`).join('');
    const details = calls.length ? `<details class="flow-calls"><summary>查看 ${Number(stage.model_call_count)} 次模型调用</summary><div class="table-scroll"><table class="flow-call-table"><thead><tr><th>调用</th><th>状态</th><th class="numeric">Token 入 / 出</th><th class="numeric">耗时</th><th class="numeric">费用</th></tr></thead><tbody>${callRows}</tbody></table></div></details>` : '<p class="flow-no-calls">该阶段没有逐调用明细。</p>';
    return `<article class="flow-stage ${stage.status === 'error' ? 'has-error' : ''}"><div class="flow-index">${Number(stage.index)}</div><div class="flow-content"><div class="flow-stage-head"><div><div class="flow-stage-title"><h3>${escapeHtml(stage.title)}</h3><span class="scope-pill">${stage.scope === 'workflow' ? '整页共享' : '当前题'}</span></div><p>${escapeHtml(stage.description)}</p></div><div class="flow-stage-cost"><strong>¥${formatCostMicros(stage.estimated_cost_micros)}</strong><span>${Number(stage.model_call_count)} 次调用</span></div></div>${details}</div></article>`;
  }).join('')}</div>`;
}

async function renderFeedbackDetail(feedbackId) {
  const body = mountShell('feedback', '反馈详情', '查看被反馈回复、相关处理流程和逐模型费用。');
  try {
    const data = await api(`/api/admin/feedback/${encodeURIComponent(feedbackId)}`);
    const isPageFeedback = data.feedback_scope === 'page';
    const traceEyebrow = isPageFeedback ? 'PAGE RECOGNITION TRACE' : 'ONE QUESTION TRACE';
    const traceTitle = isPageFeedback ? '整页识别处理流程' : '单题检索处理流程';
    const traceDescription = isPageFeedback
      ? '汇总这次上传页面在被反馈回复前发生的整页理解、框选与校验步骤。'
      : '汇总当前题的题库检索步骤；“整页共享”是同页所有题共同发生的前置处理。';
    ...
    const repriceNote = data.cost.historical_reprice_applied ? '<p class="reprice-banner">历史记录中的新模型原先没有价目，本页已按当前官方目录和原始 Token 只读补算。</p>' : '';
    body.innerHTML = `<div class="detail-topbar"><a class="back-link" href="${attr(back)}">${icon('back')}返回反馈列表</a>${archiveAction}</div><div class="detail-layout"><div class="detail-main"><section><header class="conversation-head"><h2>完整对话</h2><div class="conversation-badges">${feedbackScopeBadge(data.feedback_scope)}${statusBadge(data.rating, ratingText(data.rating))}</div></header><div class="conversation">${conversationHtml(data)}</div></section><section class="workflow-section"><header class="workflow-head"><div><span class="eyebrow">${traceEyebrow}</span><h2>${traceTitle}</h2><p>${traceDescription}</p></div>${routePill(data.cost)}</header>${repriceNote}${flowHtml(data.cost)}</section></div><aside class="detail-side"><section class="detail-section"><h2>反馈信息</h2><dl class="facts"><dt>反馈编号</dt><dd class="feedback-number">${escapeHtml(data.feedback_number)}</dd><dt>反馈范围</dt><dd>${feedbackScopeBadge(data.feedback_scope)}</dd><dt>用户</dt><dd>${escapeHtml(feedbackUserLabel(data))}</dd><dt>提交时间</dt><dd>${formatFullDate(data.created_at)}</dd><dt>处理路线</dt><dd>${escapeHtml(data.cost.route_label || data.image_route || '未识别')}</dd><dt>章节</dt><dd>${escapeHtml(data.chapter || '未确定')}</dd><dt>反馈原因</dt><dd>${data.tags.map((tag) => escapeHtml(TAG_LABELS[tag] || tag)).join('、') || '未选择'}</dd><dt>补充说明</dt><dd>${escapeHtml(data.detail || '未填写')}</dd></dl></section><section class="detail-section"><h2>${isPageFeedback ? '本页运行' : '本题运行'}</h2><dl class="facts"><dt>关联费用</dt><dd>¥${escapeHtml(data.cost.estimated_cost_cny)}</dd><dt>模型调用</dt><dd>${Number(data.cost.model_call_count)} 次</dd><dt>开始时间</dt><dd>${formatDateTime(data.cost.started_at)}</dd><dt>结束时间</dt><dd>${formatDateTime(data.cost.finished_at)}</dd>${candidateFact}<dt>流程耗时</dt><dd>${formatDuration(data.search_duration_ms)}</dd></dl></section><section class="detail-section"><h2>模型费用拆分</h2>${modelSummaryHtml(data.cost.models)}</section><section class="detail-section"><h2>处理记录</h2><form class="review-form" id="review-form"><div class="field"><label for="review-status">处理状态</label><select class="select" id="review-status" name="review_status"><option value="pending" ${data.review_status === 'pending' ? 'selected' : ''}>待处理</option><option value="resolved" ${data.review_status === 'resolved' ? 'selected' : ''}>已处理</option><option value="no_action" ${data.review_status === 'no_action' ? 'selected' : ''}>无需处理</option></select></div><div class="field"><label for="admin-note">内部备注</label><textarea class="textarea" id="admin-note" name="admin_note" maxlength="2000" placeholder="记录判断、原因或后续动作…">${escapeHtml(data.admin_note || '')}</textarea></div><button class="button button-primary" type="submit">${icon('save')}保存处理结果</button></form></section></aside></div>`;
