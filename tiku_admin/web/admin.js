...
function feedbackSummaryRows(items, { manageable = false } = {}) {
  const rows = items.map((item) => {
    ...
    return `<tr><td><span class="cell-main feedback-number">${escapeHtml(item.feedback_number)}</span></td><td><div class="feedback-topic"><span class="feedback-preview">${item.preview_image ? `<img src="${attr(item.preview_image)}" alt="题目缩略图">` : icon('image')}</span><span class="feedback-copy"><span class="cell-main">${escapeHtml(feedbackUserLabel(item))}</span><span class="cell-sub">${formatDateTime(item.created_at)}</span></span></div></td><td>${statusBadge(item.rating, ratingText(item.rating))}</td><td><div class="tag-list">${item.tags.map((tag) => `<span class="tag">${escapeHtml(TAG_LABELS[tag] || tag)}</span>`).join('') || '<span class="cell-sub">未选择原因</span>'}</div></td><td><div class="feedback-detail-text">${escapeHtml(item.detail || item.preview_text || '未填写补充说明')}</div></td><td class="numeric">¥${escapeHtml(item.cost?.estimated_cost_cny || '0.00')}</td><td>${item.archived_at ? statusBadge('archived') : statusBadge(item.review_status)}</td><td><div class="cell-actions ${item.archived_at ? 'archived-actions' : ''}">${detailLink}${actions}</div></td></tr>`;
  }).join('');
...
async function renderFeedbackDetail(feedbackId) {
  const body = mountShell('feedback', '反馈详情', '查看用户反馈、单题完整处理流程和逐模型费用。');
  try {
