...
BACKUP_ROOT = PROJECT_DIR.parent / "_backups" / PROJECT_DIR.name
...
class App:
    ...
    def _do_store(self):
        ...
        try:
            ...
            chapter = Path(rel_path).parts[0]
        except (ValueError, IndexError):
            messagebox.showerror("错误", "图片路径不在题库目录下，无法安全生成入库计划")
            return

        self.btn_search.config(state="disabled")
        self.btn_store.config(state="disabled")
        self.btn_audit_store.config(state="disabled")
        self._set_status("正在识别并生成入库计划...")
        ...
        def run():
            try:
                ...
                if not plans:
                    raise RuntimeError("未生成入库计划")
                plan = plans[0]
                if plan.status != "ready":
                    reason = plan.reason or "识别结果需要人工复核"
                    self.win.after(
                        0,
                        lambda r=reason: messagebox.showwarning("暂不写入", f"未通过自动入库检查：{r}"),
                    )
                    self._set_status("需要人工复核，未写入题库")
                    return

                bank_name = "数值荷载题库" if plan.target_bank == "main" else "字母荷载题库"
                loads_text = loads_to_display(plan.stored_loads)
                confirmed = self._ask_yes_no_from_worker(
                    "确认入库计划",
                    (
                        f"章节：{chapter}\n"
                        f"目标：{bank_name}\n"
                        f"Excel：{Path(plan.target_excel).name}\n"
                ...
                if not confirmed:
                    self._set_status("已取消入库，未写入题库")
                    return

                stamp = datetime.now().strftime("%Y%m%d_%H%M%S")
                backup_dir = BACKUP_ROOT / datetime.now().strftime("%Y-%m-%d") / f"legacy_gui_store_{stamp}"
                apply_results = apply_ready_plans([plan], dry_run=False, backup_dir=backup_dir)
                appended = sum(len(result.appended) for result in apply_results)
                skipped = sum(len(result.skipped_existing) for result in apply_results)
                if appended:
                    message = f"已写入 {bank_name}\n备份：{backup_dir}"
                    self.win.after(0, lambda m=message: messagebox.showinfo("储存成功", m))
                    self._set_status("储存完成")
                elif skipped:
                    self.win.after(0, lambda: messagebox.showinfo("无需写入", "该题已存在于题库中"))
                    self._set_status("题目已存在，未重复写入")
                else:
                    raise RuntimeError("入库流程没有写入记录")
            except Exception as exc:
                self.win.after(0, lambda e=str(exc): messagebox.showerror("储存失败", e))
                self._set_status("储存失败")
            finally:
                self.win.after(0, self._restore_buttons)
    ...
    def _do_audit_store(self):
        ...
        def run():
            try:
                ...
                backup_dir = BACKUP_ROOT / datetime.now().strftime("%Y-%m-%d") / f"legacy_gui_audit_{stamp}"

                if missing:
                    self._set_status(f"识别漏存题目中...（{len(missing)}题，只读）")
                    plans = classify_missing(
                        missing,
                        root=ROOT,
                        symbolic=symbolic,
                    ...
                    apply_results = apply_ready_plans(plans, dry_run=True, backup_dir=backup_dir)
                else:
                    plans = []
                    apply_results = []
                ...
                planned_count = sum(len(result.appended) for result in apply_results)
                ...
                self.win.after(0, lambda: messagebox.showinfo("漏存审查完成（只读）", message))
                self.win.after(0, lambda: self._set_status(f"只读审查完成：计划 {planned_count}，需复核 {review_count}"))
...
"""
    ...
    def _ask_yes_no_from_worker(self, title, message):
        """在 Tk 主线程显示确认框，并把结果返回后台任务。"""
        completed = threading.Event()
        answer = {"confirmed": False}

        def ask():
            try:
                answer["confirmed"] = messagebox.askyesno(title, message)
            finally:
                completed.set()

        self.win.after(0, ask)
        completed.wait()
