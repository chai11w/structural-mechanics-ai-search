...
- 复筛后的用户可见结果只显示最终相似度。
- 复筛结果不再强制补满 Top 3：复筛后的用户可见相似比 `>90%` 的结果全部输出，不受 3 个限制；如果没有 `>90%`，则输出 `>80%` 的前 3 个；如果一个都没有超过 80%，只输出相似比最高的 1 个。这个规则只用于复筛后的结果展示，不改变初筛候选池或未复筛输出。`_last_search.json` 会按最终显示排序重写，所以 `answer 1/2/3` 对应当前实际显示的排名。
