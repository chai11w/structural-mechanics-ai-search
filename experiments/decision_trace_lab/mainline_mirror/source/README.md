...
Intent V2 还提供受限的全局搜索兜底：只有章节判断失败、Agent 已明确提供该选项且用户明确同意时才会执行。它跨第 2–8 章收集粗筛分数 `>= 0.999` 的内容去重候选，以最多 10 路并发完成全部视觉复筛；复筛后按与普通章节一致的加权方式计算最终分（粗筛荷载分 50% + 视觉复筛分 50%），只展示最终分 `>= 0.95` 的结果并标注来源章节。结果仍是候选，必须由用户选择 `candidate_rank` 后才会读取答案。普通章节检索和现有飞书机器人不走该流程。
