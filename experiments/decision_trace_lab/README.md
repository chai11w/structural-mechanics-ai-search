...
镜像通过 `scripts/sync_mainline_mirror.py` 从明确的 Git 提交原始 blob 重建，不从当前工作区复制。当前8793镜像指向已把尺寸复筛补接到跨章节字母库全局搜索的8790主线提交 `9b13de8`（131文件哈希快照），同时保留严格六字段模型白名单、代码状态校验、唯一候选简短确认和同批候选答案改选修复。8793 使用主线的安全回答生成器和共享尺寸复筛逻辑，右侧继续旁路显示回答方式和完整评审信息；评审数据与8790运行状态相互隔离。
