...
镜像通过 `scripts/sync_mainline_mirror.py` 从明确的 Git 提交原始 blob 重建，不从当前工作区复制。当前8793镜像指向已加入唯一候选简短确认规则的8790主线提交 `90349bc`；此前的全局搜索最终分展示规则一并保留。8793 默认使用同一安全回答生成器，右侧继续旁路显示回答方式和完整评审信息。
