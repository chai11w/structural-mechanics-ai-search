...
镜像通过 `scripts/sync_mainline_mirror.py` 从明确的 Git 提交原始 blob 重建，不从当前工作区复制。当前8793镜像临时指向待真实评审的8794候选提交 `455da39`，包含状态感知安全回答、严格六字段模型白名单、代码状态校验、唯一候选简短确认和同批候选答案改选修复；8790主线已同步候选改选修复，其他候选功能仍保持原版本。8793 默认使用候选的安全回答生成器，右侧继续旁路显示回答方式和完整评审信息。
