...
镜像通过 `scripts/sync_mainline_mirror.py` 从明确的 Git 提交原始 blob 重建，不从当前工作区复制。当前8793镜像临时指向8794候选五态提交 `7288614`，用于提升8790前的独立评审；它包含已验收的安全回答 V0 和五态结构化工具结果，但不会修改8790。8793 默认使用同一安全回答生成器，右侧继续旁路显示回答方式和完整评审信息。
...
日常错误归因只保留可判断的业务结果：成功路由直接显示“选择主库/字母库”，结构分类显示具体类型，
