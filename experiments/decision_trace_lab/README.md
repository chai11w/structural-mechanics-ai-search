...
镜像通过 `scripts/sync_mainline_mirror.py` 从明确的 Git 提交原始 blob 重建，不从当前工作区复制。当前8793镜像临时指向待真实评审的8794候选提交 `cd569f6`，包含状态感知安全回答、严格六字段模型白名单、代码状态校验和唯一候选简短确认规则；8790主线仍保持原版本不变。8793 默认使用候选的安全回答生成器，右侧继续旁路显示回答方式和完整评审信息。
