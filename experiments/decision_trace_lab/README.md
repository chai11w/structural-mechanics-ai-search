...
镜像通过 `scripts/sync_mainline_mirror.py` 从明确的 Git 提交原始 blob 重建，不从当前工作区复制。当前8793镜像临时指向8794候选提交 `0baa143`，用于提升8790前的独立评审；它包含已验收的安全回答 V0、五态结构化工具结果和视觉复筛 EXIF 方向校准，但不会修改8790。8793 默认使用同一安全回答生成器，右侧继续旁路显示回答方式和完整评审信息。
