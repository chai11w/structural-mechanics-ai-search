...
class TikuAgent8790RetentionConfigTest(unittest.TestCase):
    ...
    def test_linked_release_uses_own_primary_runtime_and_external_backup(self):
        with tempfile.TemporaryDirectory(prefix="tiku release layout ") as temporary:
            root = Path(temporary)
            primary, release = root / "primary", root / "release"
            primary.mkdir()
            def git(*arguments):
            ...
            git("worktree", "add", "--detach", str(release), "HEAD")
            with patch("scripts.run_tiku_agent_8790.BASE", release), patch(
                "scripts.run_tiku_agent_8790.build_a3_runtime", return_value=object()
            ):
                options = dict(evidence_capacity=_capacity(), enable_a3_text_orientation=False,
                ...
                app = build_app(primary / ".tmp_tiku_agent_v2_prod_8790", **options)
                try:
                    self.assertEqual(app.state.checkpoint_retention_controller.repository_root, primary.resolve())
                    self.assertEqual(app.state.checkpoint_evidence_store.path.parent,
                                     primary / ".tmp_tiku_agent_v2_prod_8790")
                finally:
                    app.state.trace_event_recorder.close()
                with self.assertRaisesRegex(ValueError, "runtime root"):
                    build_app(root / "unrelated" / "runtime", **options)
                with self.assertRaisesRegex(ValueError, "outside the repository"):
                    build_app(primary / "runtime", **{**options,
                        "checkpoint_retention_backup_root": primary / "backups"})
