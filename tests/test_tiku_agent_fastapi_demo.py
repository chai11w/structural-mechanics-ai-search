...
class FastApiDemoTest(unittest.TestCase):
    ...
    def test_health_text_cookie_image_upload_and_media_token(self):
        ...
        self.assertIn("function updateComposer", page.text)
        self.assertIn("event.key==='Enter'", page.text)
        self.assertIn("!event.shiftKey", page.text)
        self.assertIn("!event.isComposing", page.text)
        self.assertIn("event.keyCode!==229", page.text)
        self.assertIn("Shift + Enter 换行", page.text)
        self.assertIn('role="status" aria-live="polite"', page.text)
        self.assertIn("function setStatus", page.text)
        self.assertIn("正在识别题图", page.text)
        self.assertIn("正在创建新对话", page.text)
        self.assertIn("function resetConversation", page.text)
        self.assertIn("function checkHealth", page.text)
        self.assertIn("网络已断开", page.text)
        self.assertIn("prefers-reduced-motion:reduce", page.text)
        self.assertIn("[hidden]{display:none!important}", page.text)
        self.assertIn("function scrollToLatest", page.text)
        self.assertIn("html,body{height:100%;overflow:hidden}", page.text)
        self.assertIn("overflow-y:auto;overscroll-behavior:contain", page.text)
        self.assertIn("conversation.scrollTo", page.text)
        self.assertNotIn("window.scrollTo", page.text)
        self.assertIn("function syncComposerSpace", page.text)
        self.assertIn("ResizeObserver", page.text)
