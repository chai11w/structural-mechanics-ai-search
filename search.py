...
SYMBOL_FAMILY_BASES = {
    "q": 0.010,      # 均布力为主: q, qL, qL²
    "force": 0.020,  # 集中力为主: F/L, F, FL, P/L, P, PL, Pa, Fp
    "moment": 0.030, # 集中弯矩为主: M/L², M/L, M
}
...
def normalize_symbol_raw(raw):
    ...
    if body in q_family:
        return _format_symbol_code(SYMBOL_FAMILY_BASES["q"], factor)
    if body in force_family:
        return _format_symbol_code(SYMBOL_FAMILY_BASES["force"], factor)
    if body in moment_family:
        return _format_symbol_code(SYMBOL_FAMILY_BASES["moment"], factor)
    return None


def normalize_raw(raw):
    symbol_code = normalize_symbol_raw(raw)
