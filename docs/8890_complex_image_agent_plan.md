...
当前进度：已建立 `experiments/complex_image_eval/` 的评测集草稿。当前包含 5 个题库标准图候选、2 张去重后的反馈输入图元数据，以及 6 张已归档的 A1/A3/旋转 fixture；6 类复杂或质量边界仍未达到最小样本数。尚未开始业务 Schema、视觉调用或路由验收，草稿不能作为生产验收集使用。
