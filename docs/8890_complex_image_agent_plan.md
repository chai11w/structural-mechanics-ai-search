...
Stage 2 影子记录链路和隔离 `8891` MVP 已接入。下一步仍不开发 Planner、LangGraph 或 DeepSeek Harness：

1. 保持 `8890` 影子运行，同时在 `8891` 收集 A1/A2/A3 真实比例、第二次千问回复、章节追问、检索成功率、延迟、令牌用量和费用；
2. 对照人工标签和最终检索结果，重点观察 A2 误放、A1/A3 误停以及第二次千问是否准确复述上游事实；
3. 根据成批数据决定是否补齐正式 Schema、增加 A3 拆解，或调整 A2 入口；不为单个新案例持续追加提示词规则；
4. 数据达标后再评估是否让 `8890` 成为权威入口，最后才讨论迁移到 `8790`。
