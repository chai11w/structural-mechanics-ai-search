...
- LangGraph 是后续 `8794` 学习线的编排实现候选，不是 `8890` 业务验证的前置条件。
...
| `8794` | LangGraph/自主 Agent 学习线 | 等 `8890` 契约稳定后复用 |
...
### Stage 8：`8794` LangGraph 学习线

状态：`PENDING`

前提：`8890` 的数据契约、工具权限、取消语义和验收样本稳定。

只用 LangGraph 重做编排和 checkpoint，先做行为等价对照，不同时增加新工具或新自主权限。只有确实需要跨请求暂停/恢复、人工确认节点和持久 checkpoint 时，才考虑将 LangGraph 用于业务线。
