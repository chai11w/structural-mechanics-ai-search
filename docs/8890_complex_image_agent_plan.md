...
- 已形成首份 6 条真实影子记录汇总：A1 2 条、A2 1 条、A3 3 条，6/6 调用成功，P50 约 6.8 秒、P95 约 8.9 秒，总令牌 12350；样本仍少且尚未完成逐条人工对照，因此不能进入 Stage 3。
...
Stage 2 影子记录链路已接入。下一步只收集和复核真实影子数据，不开发 Planner、LangGraph 或 DeepSeek Harness：

1. 保持原固定检索结果不变，收集新的真实上传，不重复付费运行已经评测过的题库原图；
2. 对照人工标签、当前外荷载筛查和最终检索结果，统计 A1/A2/A3 占比、纠正类型、解析失败、延迟和令牌用量；
3. 根据成批数据判断提示词是否需要调整，不为单个新案例继续追加规则；
4. 完成 Stage 2 报表和验收后，再决定是否补齐正式 Schema 并进入 Stage 3 权威路由。
