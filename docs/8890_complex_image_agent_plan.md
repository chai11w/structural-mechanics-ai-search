...
状态：`IN_PROGRESS`（粗区域识别已通过；2026-08-16 已新增隔离的区域内 OpenCV 精裁实验，尚未通过完整性门禁）
...
- 新增 `tiku_agent/a3_region_cropper.py` 和 `scripts/crop_a3_regions.py`。实验在已保存粗区域内联合全部有效前景，避免最大连通块丢失断开的荷载、尺寸和支座，并按相邻粗框把外扩限制在题目间隙；输出逐区域裁图、坐标清单和粗框/精裁框对照图，不调用模型或 A2。
- 两张具有正确区域 JSON 的代表图显示：三大题页的结构、荷载和尺寸基本保留，但会混入少量相邻题干；`5-2` 密集公共题干页仍受跨区手写标记和相邻标签影响。一次图像与区域 JSON 错配的输出已排除，不计入验收。当前实验不得产生 `single_ready`。
...
A3 Phase 2 的粗区域识别已通过，区域内 OpenCV 精裁已有首版但尚未通过。下一步仍不接 A2，不开发 Planner、LangGraph 或 DeepSeek Harness：

1. 在当前精裁实验中增加主体结构锚点与邻近分量归属，不能再把区域内所有前景无差别纳入最终框；
2. 使用来源匹配的授权图和区域 JSON 复验，清除相邻题干、跨区手写标记与残图，同时保留支座、荷载和尺寸；
