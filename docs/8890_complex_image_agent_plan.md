...
状态：`IN_PROGRESS`（2026-08-15 已完成首轮 3 张授权真实图片验证，均未达到验收标准）
...
- 首轮 3 张授权真实图结果：双大题页的 2 个 OpenCV 候选正确但模型输出未通过 Schema；`5-2` 局部页产生 6 个候选，其中一个大块合并 `(c)` 与 `(d)`，最终误报为仅 `(c)` 的 `single_ready`；三大题页的 3 个主体候选正确但模型输出未通过 Schema。三张均不得作为通过样本。
- 当前暴露两个独立缺口：Schema/Prompt 未定义“独立大题没有子题号”时 `member_labels/question_label` 的稳定表示；质量校验不能识别一个候选块内包含多个独立原结构。Schema 失败时日志还会丢失模型名、usage 和具体校验阶段，不利于诊断。
...
- 第一阶段代码门会阻止一个区域同时覆盖多个局部题号、重复局部题号、明显重叠区域和任何未知绑定通过。独立大题允许只有父题号而没有 `(a)` 子题号，避免旧 Schema 缺口。区域模块与共享图片编码器相关测试 13 项通过；用户已授权三张真实图重复测试，但工具审批服务连续返回 503，图片尚未实际发送且没有新增费用，因此第一步仍未完成业务验收。
...
A3 Phase 2 已完成首轮真实验证但未达标。下一步仍不接 A2，不开发 Planner、LangGraph 或 DeepSeek Harness：

1. 工具审批服务恢复后，按本轮用户授权只运行 `scripts/map_a3_regions.py` 验收三张代表图的区域 JSON 和画框预览，不运行 OpenCV 或完整 8892；
2. 第一阶段必须分别得到双大题 2 区域、共享题干 `(a)~(d)` 4 区域且 `(c)/(d)` 分开、三大题 3 个主体区域；不通过时只修区域 Prompt、解析和门禁；
3. 第一阶段通过后，才新增“每个粗区域内部运行 OpenCV”的第二阶段，并单独验收裁剪完整性；
4. 区域和裁剪分别通过后才重新串联分组、章节、选题与 A2。
