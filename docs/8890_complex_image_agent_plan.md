...
1. 先用 mock 修复独立大题的稳定 label 约定和 Schema 归一化，并让 Schema 失败日志保留脱敏阶段、模型与 usage；
2. 为“一个候选块包含多个独立结构”增加模型明确标记和代码阻断，至少返回 `uncertain`，不能再误报 `single_ready`；随后再评估是否需要调整 OpenCV 分块；
3. 完成本地回归后，重新发送已评测图片仍需用户再次确认，不默认重复付费调用；
4. 至少有代表性的双大题页、共享题干子题页和三大题页通过后，再进入 Phase 3 的 `8892` 交互状态流。
