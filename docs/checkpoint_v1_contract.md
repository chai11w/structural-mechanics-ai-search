...
阶段采集在首次 Store/图片 I/O 前保存有界 JSON 字节副本，覆盖结果、候选、章节、loads、选题及 A3 裁图几何等必要输入。副本不进入公共协议或持久化日志；上限为 512 KiB、16,000 节点、深度 12、每集合 1,000 项、每字符串 16,384 字符，超限仅拒绝采集。A3 同 unit 每次裁剪使用独立文件名，旧文件由会话清理和有限资源租约共同管理。

优化 4.5.3 将启动器的 A2/A3 采集统一接入有界异步队列，请求侧不再读证据图片、查询前驱或写 Store；`queued` 不代表 `stored`，只有实际提交后才关联 Trace。默认 128 项（含处理中任务）、8 MiB 记账字节、120 秒队列年龄、一个消费者，A3 和子 A2 共用 100ms 提交准备预算。进程内租约保护排队所需的不可变原图/裁图，清理竞争直接拒绝新采集，不影响业务授权。详细限制与验收见[优化记录](checkpoint_optimization_phase4_5.md)。Store 仍以可信提交时间计算原有 TTL；请求阶段时间线由 Trace 保留。
...
原 4.2 同步 Store 达到容量时先清理已过期且未 hold 内容。优化 4.5.4 的后台采集在容量不足时直接拒绝，
批量清理由独立维护流程执行，仍要求备份、plan/hash 和漂移检查。停止新增本次诊断证据但搜索继续，健康状态
...
即查看 fail-closed。后台自动 purge 的审计故障必须进入健康告警，由下一次有间隔的维护运行重新规划，
不在采集任务内重试。管理员主动延期或删除
