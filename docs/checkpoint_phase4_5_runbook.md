...
优化 4.5.3 启用后采集异步落库，业务响应返回时诊断可能尚未看到最后的 Checkpoint。`CAPTURE_QUEUED` 不提供可查询 ID；以已提交的 Trace 关联或受权查询结果为准。4.5.4 增加后台预算、维护互斥、熔断和有限关闭；达到容量时采集直接拒绝，由独立维护清理。health 的 pending/running/backlog 和各类丢弃计数用于区分尚未入库与永久丢失。队列限制及 4.5.5 验收见[优化记录](checkpoint_optimization_phase4_5.md)，本诊断 CLI 不负责等待或重跑采集。
