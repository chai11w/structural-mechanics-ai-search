...
状态：本地实现及隔离浏览器验收完成。2026-09-09 按用户要求隐藏独立控制面板，8910 体验页仅保留后台执行保护及既有新对话入口；后端控制 API 和协调逻辑继续保留。阶段 5 整体验收仍按
...
- 产品页面不显示“任务状态与控制”，包括其中的停止、结束、刷新和恢复按钮。DOM 与协调器保留供既有新对话及跨标签协调使用；页面启动的一次状态读取继续执行，没有新增自动重跑、后台轮询或暂停/继续。
- 控制使用独立 Web Lock，不排在普通任务请求锁后面；不支持 Web Lock 时拒绝控制。
  普通业务入口继续使用现有锁和 pending fence。
- 每次控制先保存操作编号、代次、版本、精确目标和需要对账的 fence，再发请求。
  网络中断或超时保留记录。2026-09-10 起，普通产品页在可见错误通知中提供“核对上次操作”，刷新后也会提示；已有“新对话”按钮在存在未确认命令时先核对原命令，核对成功后才允许下一次新操作。核对沿用原编号和原请求，不直接删除 journal，也不接着自动重置。独立控制面板仍保持隐藏。
  记录损坏时拒绝新控制，不猜测旧请求内容。
...
2026-09-12 会话恢复修复（本地）：普通 JSON、流式入口的 HTTP 错误和流式终态错误，缺少完整且精确的 fence ACK 时，仅显示已注册的服务错误并保留待核对记录，不消费返回状态、不清空历史，也不广播跨标签页重置。完整 ACK 的权威空状态仍按原规则处理。

本地判定过期而服务器确认仍有效时，继续保留历史及原最后操作时间，并安排最多一分钟后的会话复查；后续确认过期才清理。该复查仅针对会话有效期，不轮询执行进度、不重跑业务。恢复上传图片引用时也保留这个复查安排。`tests.test_demo_web_task_state` 覆盖缺失/错误版本/不完整/额外 ACK、完整 ACK 对照、自动复查及最终过期清理。本次只修改浏览器恢复行为，CLI、飞书及检索协议不变。

2026-09-10 新增 `tests/phase5_hidden_reset.js`，使用产品默认隐藏样式，不注入面板显示 CSS。对 reset 已提交但响应丢失分别验证可见核对按钮、刷新后再点新对话两条路径：每条均为两次同键同体请求、一个重置收据、journal 清除且没有二次 epoch 轮换；随后新对话正常轮换。两次故障加一次正常 reset 总计 5 请求、3 收据、0 模型调用，390px 无横向溢出。隔离 fixture 使用 18910 端口，未连接真实服务。可在已打开该 fixture 的专用 Playwright CLI session 中执行 `--raw run-code --filename tests/phase5_hidden_reset.js`。

下述为诊断夹具的控制链验证；`phase5_browser_controls.js` 在自己的浏览器上下文注入显示样式，临时展开已从产品页面隐藏的面板。它不代表当前用户页面仍显示该面板，原截图和交互记录属于隐藏前验收。

隐藏面板后的 113 项前端/HTTP 相关测试通过；真实 8910 浏览器独立会话确认面板不可见、既有新对话仍轮换 epoch、后台 execution context 可读，并在脚本将面板设为 open 后仍保持不可见。390px 页面无横向溢出，截图为 `output/playwright/phase5-panel-hidden-mobile.png`。本次只更新静态文件，没有重启体验服务。
