...
HTTP 出口，以及共享 `RuleRouter` 的 CLI、飞书和旧 GUI 使用点。媒体持久化仍属于相邻边界；
HTTP 异常映射已在本阶段补齐，但不把它们伪装成工具层问题。
...
- 已覆盖混合荷载真实工具链、动作/事实序列化、旧构造兼容，以及 Agent、HTTP JSON 和流式出口中
  注入英文/路径/Traceback 的安全回退。

本地验证：聚焦 Agent 测试 80 项、HTTP 测试 34 项通过，项目测试 759 项通过。下一步仍需在不上线
的前提下检查 A3 转发和其他入口的公共序列化，再决定是否进入受控模型润色或薄看门狗阶段。
...
6. 再处理媒体门禁和其他入口（A3、旧飞书、GUI、CLI）的公共序列化；
