...
这一层已在新 Agent/Web/A3 的公共投影中补齐：候选图片按整组原子交付，缺一张就不返回候选图片；
答案按 0 张、部分和全部交付区分，并在文本和协议中反映实际结果。每张媒体都要同时满足持久化文件
存在和公共解析器可解析，正常全部交付时原有文案保持不变。A3 答案媒体 0 张或部分失败时，当前单元
恢复为可重发状态。公共协议新增 `MEDIA_CANDIDATES_INCOMPLETE`、`MEDIA_ANSWERS_UNAVAILABLE` 和
`MEDIA_ANSWERS_PARTIAL` 三个媒体层 code。飞书入口不经过这层改造。
...
5. 已处理 HTTP JSON、流式进度/错误、A3 会话快照和候选/答案媒体交付的公共投影；正常成功文案保持原样。
