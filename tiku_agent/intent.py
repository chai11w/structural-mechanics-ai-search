...
允许的 intent：
- greeting：用户只是在打招呼或确认是否在线，没有同时提出搜题、选题等任务。
...
输出 JSON 格式：
{
  "intent": "greeting|search_image|set_chapter|select_question|select_candidate|resend_answer|explain_failure|cancel|unsupported",
  "image_path": null,
  "chapter": null,
  "question_index": null,
  "rank": null,
...
def parse_user_intent(
    ...
    *,
    ...
    if _is_greeting_only(clean):
        return IntentResult("greeting", source="rule_greeting")
...
def validate_intent_payload(
    ...
    *,
    ...
    if intent == "greeting":
        return IntentResult("greeting", data=data, source=source)
...
def parse_user_intent_rule_fallback(
    ...
    *,
    ...
    if _is_greeting_only(clean):
        return IntentResult("greeting", source="rule_fallback")
    if _is_cancel(clean):
...
def _is_greeting_only(text: str) -> bool:
    """Match a standalone greeting without swallowing a real task request."""
    clean = re.sub(r"[\s，。！？!?、,.~～]+", "", text.lower())
    return re.fullmatch(
        r"(?:你好|您好|哈喽|哈啰|嗨|hello|hi|hey|在吗|在不在|有人吗|早上好|上午好|下午好|晚上好)(?:啊|呀|哦|哈|呢|嘛|啦|哇)*",
        clean,
    ) is not None
