...
class RapidOcrTextPageOrienter:
    ...
    def choose_correction(self, image: Image.Image) -> TextOrientationDecision:
        candidates: list[TextOrientationCandidate] = []
        with self._engine_lock:
            for correction in (0, 90, 180, 270):
                candidate_image = rotate_clockwise(image, correction).convert("RGB")
                result = self._engine(np.asarray(candidate_image), use_cls=False)
                candidates.append(self._score_result(correction, result))
    ...
    def __call__(self, source_path: str | Path) -> Path:
        ...
        except Exception:  # noqa: BLE001 - optional shadow preprocessing fails open.
            return source

    def _score_result(self, correction: int, result: Any) -> TextOrientationCandidate:
