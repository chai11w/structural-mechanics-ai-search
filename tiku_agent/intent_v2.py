...
reject_candidates 表示用户否定当前整批候选但尚未要求执行下一次检索；continue_search 表示排除已经尝试过的候选后查下一批，不能与 ERROR 状态的 retry_search 混用。
show_candidates 只返回当前候选列表；report_answer_mismatch 只记录已发送答案不匹配，不自动删除、换题或修改题库。
只允许 ActionDecisionV2 字段；不要输出 Markdown。
输出格式：
{
  "action": "select_question|select_candidate|set_chapter|global_search|reject_candidates|continue_search|show_candidates|report_answer_mismatch|resend_answer|explain_failure|retry_search|cancel|greeting|small_talk|capability_help|out_of_scope|clarification|reject",
  "question_index": null,
  "candidate_rank": null,
...
def decide_intent_v2(
    ...
    *,
    ...
    return _authorize_or_clarify(
        _guard_global_search_consent(clean, decision),
        context,
    )
...
def _rule_decision(
    ...
    *,
    ...
    explicit_chapter = _parse_chapter_v2(text)
    if (
        explicit_chapter
        and _has_explicit_chapter_directive(text)
        and _explicit_question_index(text) is None
        and _explicit_candidate_rank(text) is None
    ):
        return ActionDecisionV2(
            action="set_chapter",
            chapter_override=explicit_chapter,
            chapter_target=_chapter_target(text),
            source="rule",
            confidence=1.0,
        )
    result_feedback = _result_feedback_decision(text, context)
...
def _global_search_reply(
    ...
    decline = _is_global_search_decline(text)
...
def _result_feedback_decision(
    text: str,
    context: ConversationContextV2,
) -> ActionDecisionV2 | None:
    has_candidates = context.candidate_count > 0 and context.phase in {
        "WAIT_CANDIDATE_CHOICE",
        "ANSWERED",
    }
    if not has_candidates:
        return None
    if _has_show_candidates_evidence(text):
        return _simple("show_candidates")
    if _has_continue_search_evidence(text):
...
def _guard_global_search_consent(
    text: str,
    decision: ActionDecisionV2,
) -> ActionDecisionV2:
    """Never treat an explicit refusal as authorization for the global tool."""

    if decision.action == "global_search" and _is_global_search_decline(text):
        return _clarification("missing_chapter", source="validator")
    return decision


def _is_global_search_decline(text: str) -> bool:
    compact = re.sub(r"[\s，。！？!?、,.]+", "", text)
    if re.fullmatch(r"(?:先)?(?:不用|不要|算了|不搜|先别搜)(?:了|吧)?", compact):
        return True

    # Do not confuse positive idioms such as “不要紧，帮我全局搜” with a refusal.
    scoped = compact.replace("不要紧", "").replace("没关系", "")
    negative = r"(?:不要|别|不用|不必|无需|不需要|不想|不能|不搜|不查|不找|不检索|先不|暂时不|取消|算了)"
    global_target = r"(?:全局(?:搜索|搜|找|查|检索)?|全题库|整个题库|所有章节|全部章节|每个章节|跨章节|兜底)"
    return bool(
        re.search(rf"{negative}.{{0,12}}{global_target}|{global_target}.{{0,12}}{negative}", scoped)
    )
...
def _has_continue_search_evidence(text: str) -> bool:
    compact = re.sub(r"[\s，。！？!?、,.]+", "", text)
    if compact in {"搜题", "继续", "再找", "再搜"}:
        return True
    return bool(re.search(r"(?:继续|重新|再|接着).{0,5}(?:搜|找|检索)|换(?:一|下)?批|再来一批", compact))
...
def _has_answer_mismatch_evidence(text: str) -> bool:
    compact = re.sub(r"[\s，。！？!?、,.]+", "", text)
    return bool(re.search(r"(?:答案|结果|解答).{0,6}(?:不对|不匹配|对不上|错了|有误)|(?:这题|候选).{0,5}(?:不是|不对).{0,5}(?:答案|结果)?", compact))
...
def _has_explicit_chapter_directive(text: str) -> bool:
    directive = re.search(r"(?:按|改成|改为|换成|换到|切到|使用|用)", text)
    if directive is None:
        return False
    prefix = text[max(0, directive.start() - 3):directive.end()]
    return re.search(r"(?:不要|别|不用|不按|取消)", prefix) is None
