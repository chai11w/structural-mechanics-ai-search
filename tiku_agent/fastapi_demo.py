...
const TEXT_TIMEOUT_MS=60000,IMAGE_TIMEOUT_MS=90000,HISTORY_TTL_MS=2*60*60*1000,HISTORY_LIMIT=50,HISTORY_KEY='tiku-agent-current-chat-v1';
let history=[];
function saveHistory(){try{localStorage.setItem(HISTORY_KEY,JSON.stringify({savedAt:Date.now(),messages:history.slice(-HISTORY_LIMIT)}))}catch(_error){/* The demo still works when browser storage is unavailable. */}}
function remember(message,me,images,imageAlt){const persistentImages=images.filter(url=>typeof url==='string'&&!url.startsWith('blob:'));history.push({message:String(message),me:Boolean(me),images:persistentImages,imageAlt:String(imageAlt)});history=history.slice(-HISTORY_LIMIT);saveHistory()}
function add(message,me=false,images=[],imageAlt='题库候选题',persist=true){empty.hidden=true;const row=document.createElement('article');row.className='message'+(me?' user':'');if(!me){const avatar=document.createElement('div');avatar.className='avatar';avatar.textContent='力';row.append(avatar)}const body=document.createElement('div');body.className='message-body';body.textContent=message;row.append(body);chat.append(row);if(images.length){const grid=document.createElement('div');grid.className='images'+(me?' user-images':'');images.forEach(url=>{const img=document.createElement('img');img.src=url;img.alt=imageAlt;img.addEventListener('error',()=>{const note=document.createElement('span');note.className='expired-image';note.textContent='图片已失效，请重新上传';img.replaceWith(note)},{once:true});grid.append(img)});chat.append(grid)}if(persist)remember(message,me,images,imageAlt);window.scrollTo({top:document.body.scrollHeight,behavior:persist?'smooth':'auto'});return row}
function restoreHistory(){try{const stored=JSON.parse(localStorage.getItem(HISTORY_KEY)||'null');if(!stored||!Array.isArray(stored.messages)||Date.now()-Number(stored.savedAt)>HISTORY_TTL_MS){localStorage.removeItem(HISTORY_KEY);return}history=stored.messages.slice(-HISTORY_LIMIT);history.forEach(item=>add(item.message,Boolean(item.me),Array.isArray(item.images)?item.images:[],item.imageAlt||'题库候选题',false))}catch(_error){history=[];localStorage.removeItem(HISTORY_KEY)}}
function clearHistory(){history=[];localStorage.removeItem(HISTORY_KEY)}
