...
function restoreHistory(){try{const stored=JSON.parse(localStorage.getItem(HISTORY_KEY)||'null'),savedAt=Number(stored?.savedAt),now=Date.now();if(!stored||!Array.isArray(stored.messages)||!Number.isFinite(savedAt)||savedAt>now+60000||now-savedAt>HISTORY_TTL_MS){localStorage.removeItem(HISTORY_KEY);return}chatHistory=stored.messages.slice(-HISTORY_LIMIT);chatHistory.forEach(item=>add(item.message,Boolean(item.me),Array.isArray(item.images)?item.images.filter(isPersistentImage):[],item.imageAlt||'题库候选题',false))}catch(_error){chatHistory=[];localStorage.removeItem(HISTORY_KEY)}}
