...
<div class="composer-wrap"><form class="composer" id="composer"><label class="attach" title="上传题图"><svg width="19" height="19" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><path d="M12 16V4m0 0L8 8m4-4 4 4M4 15v4a1 1 0 0 0 1 1h14a1 1 0 0 0 1-1v-4"/></svg><input id="file" type="file" accept="image/*"></label><textarea id="text" rows="1" placeholder="发题图，或说点什么…"></textarea><button class="send" id="send" title="发送" type="submit" disabled><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><path d="m5 12 14-7-4 14-3-5-5-2Z"/><path d="m12 14 3-3"/></svg></button></form><div class="notice">Enter 发送 · Shift + Enter 换行 · 题图与会话将在 2 小时后自动清理</div></div>
...
<script>
const chat=document.querySelector('#chat'),empty=document.querySelector('#empty'),text=document.querySelector('#text'),file=document.querySelector('#file'),form=document.querySelector('#composer'),send=document.querySelector('#send'),dropOverlay=document.querySelector('#drop-overlay');
...
function remember(message,me,images,imageAlt){const persistentImages=images.filter(url=>typeof url==='string'&&!url.startsWith('blob:'));chatHistory.push({message:String(message),me:Boolean(me),images:persistentImages,imageAlt:String(imageAlt)});chatHistory=chatHistory.slice(-HISTORY_LIMIT);saveHistory()}
function add(message,me=false,images=[],imageAlt='题库候选题',persist=true){empty.hidden=true;const row=document.createElement('article');row.className='message'+(me?' user':'');if(!me){const avatar=document.createElement('div');avatar.className='avatar';avatar.textContent='力';row.append(avatar)}const body=document.createElement('div');body.className='message-body';body.textContent=message;row.append(body);chat.append(row);if(images.length){const grid=document.createElement('div');grid.className='images'+(me?' user-images':'');images.forEach(url=>{const img=document.createElement('img');img.src=url;img.alt=imageAlt;img.addEventListener('load',()=>{if(url.startsWith('blob:'))URL.revokeObjectURL(url)},{once:true});img.addEventListener('error',()=>{if(url.startsWith('blob:'))URL.revokeObjectURL(url);const note=document.createElement('span');note.className='expired-image';note.textContent='图片已失效，请重新上传';img.replaceWith(note)},{once:true});grid.append(img)});chat.append(grid)}if(persist)remember(message,me,images,imageAlt);window.scrollTo({top:document.body.scrollHeight,behavior:persist?'smooth':'auto'});return row}
function restoreHistory(){try{const stored=JSON.parse(localStorage.getItem(HISTORY_KEY)||'null');if(!stored||!Array.isArray(stored.messages)||Date.now()-Number(stored.savedAt)>HISTORY_TTL_MS){localStorage.removeItem(HISTORY_KEY);return}chatHistory=stored.messages.slice(-HISTORY_LIMIT);chatHistory.forEach(item=>add(item.message,Boolean(item.me),Array.isArray(item.images)?item.images:[],item.imageAlt||'题库候选题',false))}catch(_error){chatHistory=[];localStorage.removeItem(HISTORY_KEY)}}
...
function busy(value){isBusy=value;text.disabled=value;file.disabled=value;updateComposer()}
...
async function sendText(){const value=text.value.trim();if(!value||isBusy)return;add(value,true);text.value='';resizeComposer();busy(true);try{const data=await request('/api/message',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({text:value})},TEXT_TIMEOUT_MS,'请求等待时间过长，请稍后重试。');add(data.text,false,data.images||[])}catch(error){add(error.message||'暂时无法处理，请再试一次。')}finally{busy(false);text.focus()}}
async function uploadImage(selected){if(isBusy)return;const validationError=validateImage(selected);if(validationError){add(validationError);file.value='';return}const preview=URL.createObjectURL(selected);add('我发了一张题图。',true,[preview],'已上传题图');const pending=add('正在识别题图，请稍等…',false,[],'题库候选题',false);file.value='';busy(true);try{const data=await request('/api/image',{method:'POST',headers:{'x-filename':selected.name,'content-type':selected.type},body:selected},IMAGE_TIMEOUT_MS,'题图识别时间过长。原图已保留，你可以直接回复“重试”。');finishPending(pending,data.text,data.images||[])}catch(error){finishPending(pending,error.message||'图片暂时无法处理，请再试一次。')}finally{busy(false);text.focus()}}
function hideDropOverlay(){dragDepth=0;dropOverlay.classList.remove('is-visible');dropOverlay.setAttribute('aria-hidden','true')}
form.addEventListener('submit',event=>{event.preventDefault();sendText()});text.addEventListener('input',()=>{resizeComposer();updateComposer()});text.addEventListener('keydown',event=>{if(event.key==='Enter'&&!event.shiftKey&&!event.isComposing&&event.keyCode!==229){event.preventDefault();sendText()}});file.addEventListener('change',()=>uploadImage(file.files[0]));document.addEventListener('dragenter',event=>{event.preventDefault();if(isBusy)return;dragDepth+=1;dropOverlay.classList.add('is-visible');dropOverlay.setAttribute('aria-hidden','false')});document.addEventListener('dragover',event=>{event.preventDefault();if(event.dataTransfer)event.dataTransfer.dropEffect=isBusy?'none':'copy'});document.addEventListener('dragleave',event=>{event.preventDefault();if(isBusy)return;dragDepth=Math.max(0,dragDepth-1);if(!dragDepth)hideDropOverlay()});document.addEventListener('drop',event=>{event.preventDefault();const selected=event.dataTransfer?.files?.[0];hideDropOverlay();if(!isBusy)uploadImage(selected)});window.addEventListener('blur',hideDropOverlay);document.querySelector('#new-chat').addEventListener('click',async()=>{await fetch('/api/reset',{method:'POST'});clearHistory();chat.replaceChildren();empty.hidden=false;text.focus()});restoreHistory();updateComposer();
</script></body></html>"""
