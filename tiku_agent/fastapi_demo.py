...
<style>.images.user-images{margin:3px 0 24px auto;width:min(76%,360px)}@media(max-width:720px){.images.user-images{width:min(88%,360px)}}</style>
<script>
...
const TEXT_TIMEOUT_MS=60000,IMAGE_TIMEOUT_MS=90000;
function add(message,me=false,images=[],imageAlt='题库候选题'){empty.hidden=true;const row=document.createElement('article');row.className='message'+(me?' user':'');if(!me){const avatar=document.createElement('div');avatar.className='avatar';avatar.textContent='力';row.append(avatar)}const body=document.createElement('div');body.className='message-body';body.textContent=message;row.append(body);chat.append(row);if(images.length){const grid=document.createElement('div');grid.className='images'+(me?' user-images':'');images.forEach(url=>{const img=document.createElement('img');img.src=url;img.alt=imageAlt;grid.append(img)});chat.append(grid)}window.scrollTo({top:document.body.scrollHeight,behavior:'smooth'});return row}
...
async function request(url,options,timeoutMs,timeoutMessage){const controller=new AbortController();const timer=setTimeout(()=>controller.abort(),timeoutMs);try{const response=await fetch(url,{...options,signal:controller.signal});const data=await response.json();if(!response.ok)throw new Error(data.detail||'暂时无法处理，请再试一次。');return data}catch(error){if(error.name==='AbortError')throw new Error(timeoutMessage);throw error}finally{clearTimeout(timer)}}
