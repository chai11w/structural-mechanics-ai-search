...
- GUI 图片检索默认走多 Agent 流程并启用复筛：Qwen 识别分类，RuleRouter 路由到主库/字母库/复核区，Zhipu 输出复筛 Top 3。章节下拉默认是“自动识别章节”；只有选择该项时才会使用 `chapter=auto`，若用户选择具体章节则完全尊重用户选择，不自动覆盖。
...
- `scripts/feishu_tiku_bot.py` 是本项目原生的专职题库飞书机器人 MVP。第二大脑项目只作为飞书接入参考，不直接复用其业务代码。当前已支持 dry-run 状态机：收图、问章节/自动章节、返回 Top 3、选择答案；真实飞书图片下载/上传封装在 `FeishuClient` 中。
...
- 飞书已接入题库维护 Agent 的自然语言入口，但保留旧快捷键优先。Agent 第一版使用 Qwen 文本模型作为严格意图大脑：常见明确命令先走本地规则秒回，只有本地规则不懂时才调用 LLM；LLM 只能输出白名单 JSON 意图，不能直接操作文件或 Excel；LLM 失败/无 key/输出非法时回退到本地规则 Router。用户可在检索出 Top 3 后说 `删除第一个`、`替换第一个答案`、`这个答案不对，换成我接下来发的图`，Agent 会用最近检索结果的精确题目路径定位；也可说 `查看 4力法 1题`、`删除 4力法 1题`，如果旧题库里该编号对应多个路径，Agent 只列出候选并要求回复序号，不会按模糊题号直接删除或替换。替换答案会先收新答案图，发完回复 `1` 进入确认页；删除和替换都必须回复 `1` 确认执行，`0` 取消。
...
- 多题图不要让 LLM 直接给 bbox 来复筛；坐标容易漂。当前做法是 OpenCV 找结构图块、按题号顺序绑定并裁图；裁图缺失时退回荷载粗筛。章节未知或不在 2-6 章范围内的题不要自动检索，等用户指定章节或单独发图。
- 飞书 Agent 的删除/替换必须基于唯一题目定位。优先使用最近检索结果的精确路径；如果只给 `章节+题号` 且匹配多个旧目录题号，必须让用户选择具体路径。不要让模型按模糊题号直接改文件或 Excel。
- 飞书 Agent 的 LLM 大脑只能做意图识别和工具调度建议，不能直接执行写操作；所有高风险动作仍由本地工具层、TargetResolver、确认页、备份和操作日志控制。
