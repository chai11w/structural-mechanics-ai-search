...
- 字母荷载第一版按三个量纲体系归一化，不把字母固定死：均布力主题、集中力主题、集中弯矩主题。同一个字母可根据荷载类型和表达式形态归入不同体系，例如 `A/L、A、AL` 可表示集中力主题，`A、AL、AL²` 可表示均布力主题，`A/L²、A/L、A` 可表示集中弯矩主题；同体系按倍数映射为保留小数编码后继续走原相似度算法。
