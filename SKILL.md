...
- 独立 Agent 的 Intent V2 在普通章节候选页支持“没有/都不是”整批否定、“继续搜/换一批”继续检索、“回到候选”和“答案不对”。继续检索只沿用当前题图、章节、路由和结构类型，排除已尝试过的粗筛候选后取下一批；候选耗尽后不再调用工具。`retry_search` 仍只用于 `ERROR` 状态，不得与结果不匹配混用。8788 飞书和 8790、8793、8794 Agent 对“支持哪些章节”共同复用同一代码事实：从 `CHAPTERS` 去掉本地目录章号后生成知识范围固定答案，并说明矩阵位移和影响线仅支持含具体外荷载的题目；禁止调用模型、工具或修改会话状态，混有搜题指令时仍走各自原业务状态机。参数式 CLI 没有自然语言对话入口，不额外增加伪问答命令。该能力不自动跨章节或降低复筛阈值。
