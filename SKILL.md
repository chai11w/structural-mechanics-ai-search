...
- 飞书候选页支持删除错题：单题候选页回复 `-1/-2/-3` 删除对应候选；多题需要先进入某一题候选页，再回复 `-1/-2/-3` 删除当前题候选。删除前必须二次确认，回复 `1` 执行、`0` 取消；执行时会先备份 Excel，再把题目/答案图片移动到 `backups/delete_question_时间戳/`，最后从主库或字母库 Excel 中删除精确匹配的题目行。
