...
如需对字母库大候选池试验 V5.2 尺寸复筛，必须显式开启：

```powershell
python scripts/multi_agent_search.py --image "D:\path\to\question.jpg" --chapter "2静定结构" --enable-dimension-filter
```
...
- 实验性尺寸层默认关闭。开启后只在 `route=symbolic`、上游已知结构类型、荷载粗筛 100% 候选 `>20` 时触发，不重复识别结构类型，只看查询图一次并缓存结果。完整尺寸与完整尺寸明确不同才硬删；单边尺寸与完整尺寸任一边或另一个单边相同时提前，不同时仍保留。拱不参与；模型失败、query 未校验、candidate 缺尺寸或全删空时都回退原候选池。
