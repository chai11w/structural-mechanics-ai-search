...
- 复筛后的用户可见结果只显示最终相似度。
- 复筛结果不再强制补满 Top 3：复筛后的用户可见相似比 `>90%` 的结果全部输出，不受 3 个限制；否则按 `>80%` 的结果最多输出 3 个；如果一个都没有超过 80%，只输出相似比最高的 1 个。若 `>90%` 结果不足 3 个，可用后续 `>80%` 结果补到最多 3 个。这个规则只用于复筛后的结果展示，不改变初筛候选池。`_last_search.json` 会按最终显示排序重写，所以 `answer 1/2/3` 对应当前实际显示的排名。
