...
- 飞书已接入题库维护 Agent 的自然语言入口，但保留旧快捷键优先。用户可在检索出 Top 3 后说 `删除第一个`、`替换第一个答案`，Agent 会用最近检索结果的精确题目路径定位；也可说 `查看 4力法 1题`、`删除 4力法 1题`，如果旧题库里该编号对应多个路径，Agent 只列出候选并要求回复序号，不会按模糊题号直接删除或替换。替换答案会先收新答案图，发完回复 `1` 进入确认页；删除和替换都必须回复 `1` 确认执行，`0` 取消。
