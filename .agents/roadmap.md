...
- 工程化梳理优先于新增自主功能：新 Agent/Web 统一输出层以及 Trace 2.2～2.5 已完成，8790/8896 精确重启、无费用协议烟测、只读 live 对照和保留 dry-run 均已通过；阶段 3.1 契约与 3.2 服务端权威快照生成已整体完成，3.3.2～3.3.4 已接入受控 HTTP 出口，3.3.5 已接入五条任务 stream 的 result/非准入 error，busy/queue 拒绝保持无状态；前端仍未接入，仍不动 8788/8794，也不提前混入阶段结果或暂停/继续。
- 3.3.1～3.3.5 已完成且尚未部署；下一步仅为 3.3.6 跨出口 parity/文档验收，形成 3.3 完成门后仍不进入 3.4。
...
3. **统一任务状态快照（IN_PROGRESS）**：为父 workflow 和子题任务提供权威的当前阶段、状态、已完成内容、允许动作和下一阶段视图，避免各入口自行拼装状态。
   ...
   - 3.3 JSON/stream 出口一致性拆为 6 个可独立回退小步：3.3.1 exact typed V1 公共映射契约、3.3.2 `/api/session`、3.3.3 HTTP 200 业务 JSON、3.3.4 受控非 stream HTTP error、3.3.5 五条任务 stream 的 result/error 均 DONE。stream success 使用 `event.data.task_state`，error 使用 `event.task_state`，不改变 progress 或增加尾随事件；响应点冻结、异常 exact pair、媒体重开及 fail-closed 语义与 HTTP 出口一致，busy/queue 拒绝保持零读取且无状态。65 项 task-state、189 项直接相关 HTTP/stream/A3/反馈及全仓 1129 项回归通过，尚未部署。下一步仅为尚未开始的 3.3.6 跨出口验收；3.4 前端消费、3.5 启用门仍为 PLANNED，阶段 4～6 继续按依赖顺序保留。
