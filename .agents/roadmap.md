...
主线是把 `8790` 固定编排做成邀请制 C 端可用的可靠搜题服务，并用 `8890/8891/8892` 隔离线验证 C 端复杂题图 A1/A2/A3 处理。A3 当前先交付“整页结构化理解 + 用户人工裁剪 + 单题 A2 交接”MVP；Paddle 自动裁剪降为 V2。`8794` 的有限自主升级继续暂停。

## Current Priority

- 8790 网页主线与管理闭环已可内测：独立管理员后台 8795（邀请码生命周期、单码用量/费用、反馈）已完成并接入 8790 控制库；用户端继续邀请码登录，不迁移账号体系。
- 当前开发重点是 A3 人工裁剪 MVP：先固化整页结构化理解契约和固定样本测试，再实现题目目录、选择、按用户目标次数裁剪、A2 交接和返回列表。Paddle 试验结论保留，不再作为当前阻塞项。
...
- 不改造 8788，不自主化题库写操作。
- 人工裁剪 MVP 完成前不继续优化 Paddle splitter，不引入系统自动裁剪。
...
- A3 MVP Phase 2 整页结构化理解：IN_PROGRESS（Prompt 样本验证已通过核心语义，待生产契约、严格解析、`unit_id` 绑定和固定测试）
- A3 MVP Phase 3 人工裁剪交互：PENDING（题目目录、选择、裁剪次数、先搜哪题、返回列表）
- A3 MVP Phase 4 单题 A2 交接：PENDING（裁剪图绑定 `a2_context_text`，A2 重新判断章节、结构和荷载）
