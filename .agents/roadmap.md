...
- 工程化梳理优先于新增自主功能：新 Agent/Web 统一输出层以及 Trace 2.2～2.5 已完成，8790/8896 精确重启、无费用协议烟测、只读 live 对照和保留 dry-run 均已通过；阶段 3.1 契约与 3.2 服务端权威快照生成已整体完成，3.3.2 已接入 `/api/session`，3.3.3 已接入 HTTP 200 业务 JSON；受控 HTTP error、stream 和前端仍未接入，仍不动 8788/8794，也不提前混入阶段结果或暂停/继续。
- 3.3.1 公共映射契约、3.3.2 `/api/session` 与 3.3.3 HTTP 200 业务 JSON 接入已完成；用户已授权在新对话依次完成 3.3.4 受控 HTTP error、3.3.5 stream result/error 和 3.3.6 跨出口 parity/文档验收，形成 3.3 完成门后仍不进入 3.4。
...
3. **统一任务状态快照（IN_PROGRESS）**：为父 workflow 和子题任务提供权威的当前阶段、状态、已完成内容、允许动作和下一阶段视图，避免各入口自行拼装状态。
   ...
   - 3.3 JSON/stream 出口一致性拆为 6 个可独立回退小步：3.3.1 exact typed V1 公共映射契约 DONE；3.3.2 `/api/session` DONE，成功载荷保留旧 `session` 并新增根级 `task_state`；3.3.3 HTTP 200 业务 JSON DONE，`/api/message`、`/api/image`、启用 A3 时的 `/api/a3/select` 与 `/api/reset` 返回根级 `task_state`，A2 单锁单读、A3→A2 双锁父子各单读并冻结 legacy/typed/media，初始和媒体重开组合均 fail-closed，空任务取消返回 canonical empty、活跃取消冻结 `CANCELLED`。65 项 task-state、193 项相关出口及全仓 1072 项回归通过，尚未部署。下一步固定为 3.3.4 受控 HTTP error；3.3.5 stream result/error、3.3.6 跨出口验收也尚未开始。3.4 前端消费、3.5 启用门仍为 PLANNED，阶段 4～6 继续按依赖顺序保留。
