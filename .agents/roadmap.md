...
- 工程化梳理优先于新增自主功能：新 Agent/Web 统一输出层以及 Trace 2.2～2.5 已完成，8790/8896 精确重启、无费用协议烟测、只读 live 对照和保留 dry-run 均已通过；阶段 3.1 契约与 3.2 服务端权威快照生成已整体完成，3.3.2 已接入 `/api/session`，3.3.3 已接入 HTTP 200 业务 JSON，3.3.4 已接入精确五路径的受控非 stream HTTP error；stream 和前端仍未接入，仍不动 8788/8794，也不提前混入阶段结果或暂停/继续。
- 3.3.1～3.3.4 已完成且尚未部署；下一步依次为 3.3.5 stream result/error 和 3.3.6 跨出口 parity/文档验收，形成 3.3 完成门后仍不进入 3.4。
...
3. **统一任务状态快照（IN_PROGRESS）**：为父 workflow 和子题任务提供权威的当前阶段、状态、已完成内容、允许动作和下一阶段视图，避免各入口自行拼装状态。
   ...
   - 3.3 JSON/stream 出口一致性拆为 6 个可独立回退小步：3.3.1 exact typed V1 公共映射契约 DONE；3.3.2 `/api/session` DONE；3.3.3 HTTP 200 业务 JSON DONE；3.3.4 受控非 stream HTTP error DONE，精确覆盖具有既有会话的 `/api/session`、`/api/message`、`/api/image`、`/api/a3/select` 与 `/api/reset`。错误点冻结并携带 legacy/typed，边界不事后重读；missing/expired 返回 canonical empty，不可读、purge 或映射失败返回脱敏 `INCONSISTENT`，A3 部分清理失败按 post-attempt A3→A2 read-set 暴露 `ORPHAN_CHILD_TASK`。无会话、队列准入拒绝及非精确五路径保持零补读，busy 即使误带 typed 也由边界丢弃。65 项 task-state、206 项相关 HTTP/runtime 出口及全仓 1116 项回归通过，尚未部署。下一步固定为 3.3.5 stream result/error；3.3.6 跨出口验收也尚未开始。3.4 前端消费、3.5 启用门仍为 PLANNED，阶段 4～6 继续按依赖顺序保留。
