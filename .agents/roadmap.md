...
- 生产继续收集真实失败记录：已覆盖单题、多题、选题和答案的五个生产请求，33/33 Checkpoint 成功且 Trace/unit 归属一致。阶段 5 专用分支已完成 5.1～5.3，本地全仓 1504 项通过，未上线；5.4～5.5 待实现，不自动推进阶段 6 或暂停/继续。
...
5. **幂等执行与父子任务控制（5.1～5.3 LOCAL DONE / 5.4 IN PROGRESS / 5.5 PLANNED）**：统一任务版本、幂等键、执行锁和 A3→A2 父子生命周期，避免重复点击、网络重试或恢复造成重复执行和重复计费。
   ...
   - 5.4 进行中：已完成调用持久记录、费用待写/精确补账、子题状态与收据同事务、父收尾确认及原子文件发布；全仓 1521 项通过，后续文件专项 6 项通过。显式恢复、多题部分恢复、控制语义及维护入口尚未完成，见 [恢复契约](../docs/phase5_4_recovery_contract.md)。随后进入 5.5 集成与发布验收；不承诺供应商恰好一次、未知效果自动恢复或整套 A01～A18 已通过。
