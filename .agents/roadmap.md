...
- 2026-09-10：阶段 5 的 5.1～5.5 已完成，1623 项回归通过，按用户单独授权发布到 8790（固定提交 `3482970e9d4ba76c299314a798c240b4905bd2da`）。已迁移 1 个父会话和 1 个等待选题的子会话，同日快进合入本地主线 `codex/mainline-bounded-autonomy-v1`，未推送；生产仍使用原固定 release。本次未额外调用真实模型，不自动推进阶段 6 或暂停/继续。详见 [发布记录](../docs/phase5_8790_release.md)。此前阶段 4 已覆盖单题、多题、选题和答案的五个生产请求，33/33 Checkpoint 成功且 Trace/unit 归属一致。
- 2026-09-11/12：Web 会话恢复修复批次（7 个提交，独立于 5.x 编号）DONE，并按用户单独授权分两次发布到 8790（现役 `0be38081d4c81e920121fae08172843cf10aca17`，回退锚点 `ebf6753`、`3482970` 的 release 目录保留），随后快进合入本地主线，未推送。内容：过期判定改由服务端裁决、会话时长单一出处、失败照实显示、Trace 锁竞争重试。全仓 1629 项回归通过，并完成隔离实例真实端到端（6 次模型调用）与 8790 真实使用验证。
...
5. **幂等执行与父子任务控制（5.1～5.5 DONE，已上线 8790）**：统一任务版本、幂等键、执行锁和 A3→A2 父子生命周期，避免重复点击、网络重试或恢复造成重复执行和重复计费。
   ...
   - 5.4/5.5：调用与费用交接、父子收据、逐题恢复、配置版本、迁移/回退及 A01～A18 隔离验收已完成。2026-09-10 复验另补费用在抢占/出队/发送前的重新准入及隐藏控制面板后的可见核对入口；详见 [验收矩阵](../docs/phase5_acceptance_matrix.md)、[恢复契约](../docs/phase5_4_recovery_contract.md) 和 [浏览器控制](../docs/phase5_browser_controls.md)。8790 已按单独授权完成生产迁移与发布，真实模型采样不包含在本次只读上线冒烟中；不承诺供应商恰好一次或未知效果自动恢复。
   - 会话恢复修复批次（DONE / 已上线 8790，编号独立于上面的 5.x）：由生产问题而非原计划驱动——过期会话先读上下文再 reset、服务端无此会话时保留仍然新鲜的本地对话并标记“已失效”、会话时长收敛到 `conversation_ttl.py` 单一出处并由服务端下发、失败按服务端注册表照实显示、Trace 锁竞争改为重试。发布与回退资料见 `F:\cc\_backups\7-题库检索\2026-09-11\`（备份报告标题里的“阶段 5.1/5.2”字样与本路线图的 5.x 编号无关）。
