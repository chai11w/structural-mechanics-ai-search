...
- 工程化梳理优先于新增自主功能：新 Agent/Web 统一输出层以及 Trace 2.2～2.5 已完成，8790/8896 精确重启、无费用协议烟测、只读 live 对照和保留 dry-run 均已通过；阶段 3.1 契约和 3.2.1 纯构造器已完成但尚未接入运行时，仍不动 8788/8794，也不提前混入阶段结果或暂停/继续。
...
3. **统一任务状态快照（IN_PROGRESS）**：为父 workflow 和子题任务提供权威的当前阶段、状态、已完成内容、允许动作和下一阶段视图，避免各入口自行拼装状态。
   ...
   - 3.2.2 锁内权威读取：NEXT；在 A3→A2 锁序内一次冻结父子状态，区分缺失/不可读并证明入口和文件证据，再调用 3.2.1 纯构造器；当前暂停，暂不开始。
   - 3.3～3.5 JSON/stream 出口、前端消费和启用门：PLANNED；阶段 4～6 仍按依赖顺序保留。
