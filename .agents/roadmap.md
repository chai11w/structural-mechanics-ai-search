...
- 工程化梳理优先于新增自主功能：新 Agent/Web 统一输出层已完成；trace Phase 1 已完整盘点 8 套记录、错误/反馈链及契约，下一步按依赖推进 Trace Context、状态快照、阶段结果、幂等父子控制和长任务解耦；暂停/继续不是当前直接开发项。
...
2. **统一 trace、日志、错误和用户反馈（IN PROGRESS）**：Phase 1 已完成 active/legacy 路径、8 套记录、错误 emitter/漏斗、重复/丢失、反馈信任/费用 join 及最小契约审计，见 [`现状矩阵`](../docs/trace_phase1_observability_inventory.md) 与 [`Trace V1 契约`](../docs/trace_phase1_audit.md)；下一批只实现 Trace Context 与 ID 传播，不提前重构状态机。
...
2. **2.2 Trace Context 与 ID 传播（NEXT）**：服务端生成 `trace_id`，加法贯通 HTTP、线程、A3、A2、tool 和 cost scope；先不改业务状态机。
3. **2.3 结构化事件与终态一致性（PLANNED）**：双写最小事件流，统一 JSON/stream、A2/A3 和媒体后处理的 terminal event，并给 fail-open writer 加丢失/失败健康计数。
4. **2.4 权威响应与反馈绑定（PLANNED）**：保存 `response_id`，反馈按 `rated_response_id` 校验所有权，消除可选 conversation 绕过；旧反馈继续只读兼容。
5. **2.5 查询、保留与切换（PLANNED）**：8795 增加按 trace 的只读时间线，接通反馈保留期/周期清理，核对新旧双写后再停止启发式主查询；不删除历史账本。
