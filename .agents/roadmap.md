...
5. **幂等执行与父子任务控制（5.1～5.3 LOCAL DONE / 5.4 IN PROGRESS / 5.5 PLANNED）**：统一任务版本、幂等键、执行锁和 A3→A2 父子生命周期，避免重复点击、网络重试或恢复造成重复执行和重复计费。
   ...
   - 5.4 进行中：已实现调用持久记录、费用待写/精确补账、父子收据事务、不可变文件、runtime/HTTP 显式恢复和在途停止/reset，以及逐题检查/批次收据、部分准备恢复和输入/模型版本校验。新增只读诊断与带审阅计划/备份的费用对账、孤儿清理和容量回收入口；该批全仓 1557 项及后续 CLI 参数收口相关 9 项通过。浏览器接线、最终配置审查和综合演练仍待完成，见 [恢复契约](../docs/phase5_4_recovery_contract.md) 与 [运维说明](../docs/phase5_maintenance.md)。随后完成 5.5 集成与发布验收；不承诺供应商恰好一次、未知效果自动恢复或整套 A01～A18 已通过。
