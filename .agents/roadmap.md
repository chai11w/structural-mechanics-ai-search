...
- 当前开发重点是 A3 人工裁剪 MVP：整页 Prompt、固定回归和独立 parser 已通过，下一步实现题目目录、选择、按用户目标次数裁剪、A2 交接和返回列表。Paddle 试验结论保留，不再作为当前阻塞项。
...
- A3 MVP Phase 3 人工裁剪交互：IN_PROGRESS（题目目录、选择、裁剪次数、先搜哪题、返回列表）
