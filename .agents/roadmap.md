...
- 工程化梳理优先于新增自主功能：新 Agent/Web 统一输出层以及 Trace 2.2～2.5 已完成，8790/8896 精确重启、无费用协议烟测、只读 live 对照和保留 dry-run 均已通过；阶段 3.1 契约、3.2.1 纯构造器和 3.2.2 锁内 runtime wrapper 已完成，但尚未接入 HTTP/stream/前端，仍不动 8788/8794，也不提前混入阶段结果或暂停/继续。
...
3. **统一任务状态快照（IN_PROGRESS）**：为父 workflow 和子题任务提供权威的当前阶段、状态、已完成内容、允许动作和下一阶段视图，避免各入口自行拼装状态。
   ...
   - 3.3 JSON/stream 出口一致性：NEXT；3.4 前端消费、3.5 启用门仍为 PLANNED，阶段 4～6 继续按依赖顺序保留。
