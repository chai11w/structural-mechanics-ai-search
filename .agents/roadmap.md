...
5. **幂等执行与父子任务控制（5.1～5.5 LOCAL DONE，未上线）**：统一任务版本、幂等键、执行锁和 A3→A2 父子生命周期，避免重复点击、网络重试或恢复造成重复执行和重复计费。
   ...
   - 5.4/5.5：调用与费用交接、父子收据、逐题恢复、配置版本、迁移/回退及 A01～A18 隔离验收已完成。2026-09-10 复验另补费用在抢占/出队/发送前的重新准入及隐藏控制面板后的可见核对入口；详见 [验收矩阵](../docs/phase5_acceptance_matrix.md)、[恢复契约](../docs/phase5_4_recovery_contract.md) 和 [浏览器控制](../docs/phase5_browser_controls.md)。生产迁移、真实模型采样与发布另行安排，不承诺供应商恰好一次或未知效果自动恢复。
