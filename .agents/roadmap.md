...
5. **幂等执行与父子任务控制（5.1～5.3 LOCAL DONE / 5.4 IN PROGRESS / 5.5 PLANNED）**：统一任务版本、幂等键、执行锁和 A3→A2 父子生命周期，避免重复点击、网络重试或恢复造成重复执行和重复计费。
   ...
   - 5.4 进行中：已实现调用持久记录、费用待写/精确补账、子题状态与收据同事务、父收尾确认、原子文件发布，以及 runtime/HTTP 显式恢复和在途停止/reset；控制批次全仓 1529 项通过。后续加入逐题检查与批次收据、部分准备恢复和输入/模型版本校验，专项 20 项及最终全仓 1537 项通过。浏览器接线、运维对账/清理及最终配置审查仍待完成，见 [恢复契约](../docs/phase5_4_recovery_contract.md)。随后完成 5.5 集成与发布验收；不承诺供应商恰好一次、未知效果自动恢复或整套 A01～A18 已通过。
