...
- 工程化梳理优先于新增自主功能：新 Agent/Web 统一输出层已完成实现、回归与运行核验；统一 trace 第一阶段审计/契约已完成，下一步按依赖推进 Trace Context、状态快照、阶段结果、幂等父子控制和长任务解耦；完整暂停/继续不是当前直接开发项。
...
2. **统一 trace、日志、错误和用户反馈（IN PROGRESS）**：Phase 1 已完成现有 ID/关联断点审计、最小 Trace V1 契约和验收场景，见 [`docs/trace_phase1_audit.md`](../docs/trace_phase1_audit.md)；下一步只实现 Trace Context 与 ID 传播，不提前重构状态机。
