...
主线是把 `8790` 固定编排做成邀请制 C 端可用的可靠搜题服务，并用隔离端口验证 C 端复杂题图 A1/A2/A3 处理。A3-V1 的“整页理解 + GLM 裁图 + 用户多选/单题自动下行 + Qwen 双门禁 + 人工回退”已提升到 `8896`；Paddle splitter 继续作为 V2 延后。`8794` 的有限自主升级继续暂停。

## Current Priority

- 8790 网页主线与管理闭环已可内测：独立管理员后台 8795（邀请码生命周期、单码用量/费用、反馈）已完成并接入 8790 控制库；用户端继续邀请码登录，不迁移账号体系。
- A3-V1 已提升到 8896，8897 暂留独立回退；当前重点是 8896 真实样本验收和失败记录收集。Paddle 试验结论保留，不再作为当前阻塞项。
...
- A3 V1 GLM 整页裁图、选择性并发校验、单题自动下行和人工回退：DONE（已提升 8896）
