...
- `tiku_agent/fastapi_demo.py` 与 `scripts/run_tiku_agent_demo.py`：独立 FastAPI 本地 demo 已实现，默认仅监听 `127.0.0.1:8790`，用 session cookie 连接持久会话；受控图片令牌避免向浏览器暴露本地路径。真实 HTTP 图片入口已验证。
- FastAPI demo 使用桌面聊天布局：侧栏、新对话、居中消息流和底部输入框；`新对话` 会清除当前 session 和临时文件。
- `tiku_agent/tools.py`：首轮 scope 只判单题/多题；单题同时返回荷载/章节并直接检索，多题才调用详细题号/bbox/逐题荷载识别和裁图准备。重复同标注外荷载必须逐项保留；`unknown` 章节会转入询问章节，不会检索虚构章节。
...
- 复筛展示阈值和默认数量只定义在 `search.py`：90 分以上全部显示；否则 80 分以上最多显示 3 张；飞书、Agent 与 CLI 共享该默认策略。
