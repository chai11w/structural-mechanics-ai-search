...
2. 先讨论并验证单个视觉响应异常的恢复策略；推荐在同一 `global_search` 高层动作内由代码至多重试一次，不降低阈值、不开放自由工具循环。当前整批安全失败虽无错误执行，但会把会话留在 `ERROR`。
3. 完成恢复性验证和非原文件候选人工复核后，再在不接触线上 8790 的隔离 V2 入口做端到端人工试用，最后讨论部署；普通章节检索、现有飞书和 LangGraph 边界保持不变。
