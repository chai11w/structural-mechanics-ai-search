...
- 首轮千问结果尚不具备启用条件：人工审查发现5条自动通过回答使用“专注解答”“高效解题/解决问题”等偏宽表述，另有3条寒暄虽通过但不够简洁，需要先收紧提示词和语义校验再复测。
...
1. 讨论并收紧寒暄提示与越权能力语义校验：寒暄只做简短回应，拒绝“解答/解题/解决问题”等暗示直接求解能力的表述。
2. 使用同一10题×3次重新试跑，与本轮27/30自动通过及人工问题基线比较；通过后再运行38条完整离线验收。
3. 完整验收后才讨论在隔离8794启用；随后统一工具结果为五态协议，并定义计划/权限协议运行影子规划。
