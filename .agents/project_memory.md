...
- 8794已实现只记录不执行的影子规划、状态权限审核和纯代码语义授权闸门；旧7阶段×15话术矩阵仅保留为结构压力测试。35条金标准从`Agent.handle_text()`真实进入，并成对比较开关影子规划。最新三轮千问共105次中99次符合金标准，固定路径误触发Planner=0、回答/状态/原工具调用差异=0；Planner 10次提出人工禁止动作均被挡住，实际放行越权=0，`needs_confirmation`=11。剩余6次来自固定业务层稳定误判：“帮我看看这题哪一章”被拒绝、“有没有更接近的”在答案后直接否定整批候选。
- 语义闸门逐动作核对用户原话，覆盖全部11个Planner动作；模型补充关键词会被拆成明确/推测两组，并记录动作证据、代码确定性和`requires_confirmation`。影子日志schema升至v2。明确组合动作6条探针全部被既有固定业务层先处理，当前真实入口金标准因此没有可执行影子计划；这属于Planner准入范围问题，不是把闸门降级为宽松猜测的理由。
- 已新增40条复杂请求准入测试集及真实入口+独立诊断Planner评估器。完整三轮120次基线中，60次应观察的顺序/条件请求真实进入Planner为0，30次原子快速路径误入为0，开关影子可见差异为0；诊断Planner完整覆盖复杂目标36/60次。修正候选编号后，顺序组6/10类连续三轮完整且权限/语义可放行（18/30），另外4类会丢步骤或安全延后未来候选选择。
- 条件组暴露出比准入更优先的安全缺口：30次聚焦复验中固定Intent有27次直接执行/响应条件动作，12次调用真实检索工具；独立诊断Planner有25次通过当前语义闸门，被统计为“条件尚未满足却视为直接授权”的风险。当前Plan协议只有顺序steps和stop_condition，没有条件/分支前提，条件请求只能观察，不能作为未来执行候选。
