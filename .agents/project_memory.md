...
- V2 主线在独立工作区 `F:\cc\7-题库检索-intent-v2`；本工作区从该主线建立 `codex/global-search-fallback`，只开发严格全局搜索兜底且不启动服务。`data/feishu_chapter_failure_log.jsonl` 用户日志改动继续保留并禁止提交。
...
- V1 Intent 继续作为默认入口；V2 已通过显式 `intent_version="v2"` 接入隔离 Agent/Runtime，严格区分题号与候选排名并支持专业交流、追问、失败恢复和下一张章节，但未替换默认入口。
- 严格全局搜索四个小门已完成：协议增加无参数 `global_search`，权限要求 `WAIT_CHAPTER`、有效题图和已提供兜底选项；规则层可识别明确同意、拒绝、含糊回应和章节优先，模型输出仍由代码复核授权证据。跨章节工具、状态授权、SQLite 恢复、Agent 编排和结果渲染已连接隔离 V2，尚未部署到线上 8790。
...
- 隔离运行时已覆盖未经邀请拒绝、授权后单/多/无结果、无结果后指定章节、按 `candidate_rank` 取答案和跨重启恢复；全量197项测试通过。真实题库生产工具单题冒烟返回1个严格候选，证明链路可运行但不能外推准确率。
...
- 尚未把V2设为默认，也未连接新飞书机器人或建立真实流量影子统计；当前只允许在8791和独立运行目录试用。
- 严格全局搜索尚未部署到线上 8790；还缺少更广的裁图、截图、模糊图和跨章近似题样本，以及对非同文件高分结果的人工语义复核。
...
- 自 2026-07-14 起停止新增 V1 测试、对比和功能维护；历史 V1 指标只保留作背景。V1 代码在 V2 正式部署前暂作回退，部署稳定后另开小步删除，不与当前恢复性改动混做。
...
2. 单次受控重试已实现；继续验证重试成功、重试后仍失败、正常低分不重试和调用量统计，然后在隔离 V2 入口做端到端人工试用。
3. V2 部署稳定后单独删除 V1 开关、代码和旧运行目录；删除前保留短期回退，普通章节检索、现有飞书和 LangGraph 边界保持不变。
