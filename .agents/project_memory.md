...
- 8790 读取 8795 控制库认证；A1/A2/A3 与子 A2 统一计费并按题汇总流程/反馈。小规模内测版已提交并推送，但 live 尚未重启，当前 8790 仍是旧进程。
- 待发布版恢复有界队列：默认 1 个运行、2 个排队、55 秒等待；FIFO、防插队、流关闭撤队，同会话/同锁分片不重复占运行槽。
- 8795 仍是可替换的过渡后台；待发布版已厘清页/题、模型调用/请求次数、记录时间/页面等待/反馈时间及“模型估算费用”文案，不在本轮增加 Trace 页面。
- Trace/Response 主链独立于 8795。live 只读审计中 422 条 Trace 均有唯一终态，3 条 Response 均能对应终态；419 条早期 Trace 产生于 Response Store 启用前。
- 8 个 live SQLite 库的 `quick_check`、`integrity_check` 与 52 项时间、费用、Token、反馈、Trace/Response 对应校验通过；现有 5 条反馈均为旧版未绑定数据，新反馈绑定仍缺真实 live 样本。
- 小规模内测版全仓 1003 项回归通过；Node、PowerShell、敏感信息和 diff 检查通过。仓库外受限 Git Bundle、8 库在线副本及控制库/密钥成对备份已验证。
- live 8795 控制目录仍继承宽泛本地修改权限；收紧 ACL、精确冷启动 8795→8790、真实闭环烟测及创建测试邀请码尚未执行。
...
- 8790 有界队列、请求体限长、邀请码登录限速和安全客户端地址已实现；8795 管理登录同样限长、限速，邀请码永久删除在费用库损坏/缺表/不可读时 fail-closed。
...
- 阶段 3.1 已冻结 `TaskStateSnapshotV1` 公共结构、权威源、阶段/动作/里程碑、unit 互斥、候选 generation 和 fail-closed code；尚未接入运行时。

## In Progress

- 小规模内测发布停在 live 闸门前：需确认后收紧 8795 控制目录 ACL，精确冷启动 8795→8790，并完成至少一个巡检周期和受保护端口对照。
- 发布后需用专用邀请码完成 A2、A3、追问、正/负反馈、费用展示和停用失效闭环，补齐新 Response 绑定的真实证据。
- 阶段 3.2 权威构造器暂停，不与本轮内测发布混做。

## Not Implemented

- Cloudflare Access、边缘登录限速和测试者邮箱名单仍需账户侧配置；应用内限速不能替代边缘策略。
- 8795 尚未提供 Trace/Response 诊断 UI；本轮明确不增加，未来若成为整体后台再以只读消费者接入。
- 阶段 3.2～3.5 出口与启用门、阶段 4～6 尚未实现。
...
- 8795 控制目录当前允许宽泛本地账户修改；在收紧 ACL 前不应发放公网测试邀请码。
- 新版反馈绑定已有自动化覆盖，但 live 5 条反馈均为旧版未绑定数据；必须用发布后的真实闭环确认后台显示“服务端回复已核对”。
...
1. 用户确认 ACL 影响后，按 `docs/8790_8795_public_beta_runbook.md` 收紧 8795 控制目录权限并精确冷启动 8795→8790；等待巡检并核对受保护端口不变。
2. 完成专用邀请码真实闭环，再配置 Cloudflare Access/边缘限速并为 2～3 名测试者创建独立 7 天邀请码。
3. 观察 24～48 小时，以成功率、排队、A3 裁图、候选质量、反馈错绑和估算费用决定下一步；不先扩建 8795 或继续阶段 3.2。
