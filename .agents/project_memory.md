...
- 8793 是长期复用的观测与人工评审层，当前基于 8790 提升提交 `a771758` 的 103 文件哈希镜像运行，独立状态位于 `.tmp_review_tiku_prod_8793`，运行身份为 `review-8793-prod`，评审侧栏与标签数据均保留。
...
- 8794候选代码已完成五态结构化工具结果并受控重启，当前 Python PID 33132；健康检查、安全回答和固定业务快速路径均已真实验收。后续每个阶段仍先由8793评审，再独立提升到8790。
...
- 8794固定状态机已按五态消费结果：只有工具故障进入错误态，需要输入返回澄清，允许的部分结果继续安全降级，答案未命中保留候选选择态。聚焦65项、全量260项测试通过。

## In Progress

- 8794五态工具结果代码、自动测试、受控重启和真实冒烟均已完成；当前待进入8793评审，再开始影子规划。
...
1. 轮换用户级 `DASHSCOPE_API_KEY` 与 `ZHIPUAI_API_KEY`，再验证8794健康和模型调用。
2. 用8793保留的评审层验收五态候选；不把评审侧栏合入8794。
