...
- 每次搜题模型费用台账已只部署到8790，生产数据库为 `.tmp_tiku_agent_v2_prod_8790/model_costs.sqlite3`；8788、8793和8794均未接入。
...
1. 先让8790静默积累至少50次真实搜题费用，再用报表核验均值、P95和异常阈值；暂不把费用提示暴露给用户。
2. 根据私测量确定每日预算提醒和管理员后台展示范围。
