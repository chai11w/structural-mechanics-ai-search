...
- Intent V2 已完成旁路动作协议、权限矩阵、净化上下文、受约束推理链、简洁专业交流外壳及 V1/V2 成对评测器；未改 V1 Prompt、`agent.py`、状态机或检索链路。40 条开发回归上 V1-full 为 55%，V2-full 为 100%，但该集合已参与实现，只能用于回归。
- Blind 01 最终回归 V2 为 19/20、危险执行 0；Blind 02 首次 V2 为 9/16、危险执行 2，安全门修复后两次均为 10/16、危险执行 0。按“常见90%/长尾10%”假设的示例加权准确率为 91.75%，但该流量比例尚未实证；详见两份 blind result 文档。
...
1. 与用户审查“常见/上下文/长尾”分层和权重，建立更接近真实使用的整体指标；危险执行继续单独要求为 0。
2. 冻结未参与修复的 Blind 03，验证窄安全门的泛化，不针对剩余长尾失败加词表。
3. 整体指标和新盲测达标后再讨论 Agent/SQLite 接入；LangGraph仍留到动作协议稳定后评估。
