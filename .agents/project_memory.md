...
- 独立 FastAPI demo 监听 `127.0.0.1:8790`；具备 cookie 会话、绑定会话的媒体令牌、上传预览、新对话清理、等待提示、瞬时错误重试、前端超时和安全错误提示。上传原图使用会话目录中的稳定 `/api/upload/` 地址写入本地历史，刷新或 Demo 重启后两小时内仍可恢复；原始网络异常不会直接显示 `Failed to fetch`。桌面布局锁定整页高度，左侧栏、顶部栏和底部输入框固定，仅中间消息区独立滚动。
