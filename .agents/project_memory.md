...
- Intent V2 已完成方案审查、`ActionDecisionV2` 协议、状态—动作权限矩阵、旁路 `ConversationContextV2` 和受约束 `intent_v2.py`；首批 12 组已批准困难种子与 28 组平衡扩展草案可组合为 40 条逻辑评测集。离线评测器已支持 V1/V2 规则层与 full 成对报告，当前均为旁路实现，没有改动 V1 Prompt、`agent.py`、V1 状态机或检索链路。
...
- `ConversationContextV2` 和 Intent V2 尚未接入 `AgentState` 运行入口或 SQLite 持久化；尚未实现歧义追问/专业交流的用户回复、一次性 `pending_chapter` 实际消费与 V1/V2 影子对照。
...
2. 冻结开发回归集后补充未参与实现的盲测改写集，复跑同配置 `V1-full` / `V2-full` 并验证模型稳定性，不能只用开发集 100% 宣称成熟。
