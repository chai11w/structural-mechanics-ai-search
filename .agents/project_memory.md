...
- 8793 是长期复用的观测与人工评审层，当前为提升前验收而镜像8794最新候选的105文件哈希快照，独立状态位于 `.tmp_review_tiku_prod_8793`，运行身份为 `review-8793-prod`，评审侧栏与标签数据均保留；8790仍运行原稳定主线。
...
- 8794候选已完成五态结构化工具结果与视觉复筛EXIF方向校准并受控重启，当前 Python PID 37644；8793已同步该候选。后续每个阶段仍先由8793评审，再独立提升到8790。
...
- 8794九个业务工具及候选动作解析器已统一返回带 `tool`、`outcome`、`code`、`data`、`next_state`、`completed`、`retryable`、`error_category` 的结果；五态为 `SUCCESS / NO_MATCH / NEEDS_INPUT / PARTIAL / TOOL_ERROR`，旧 `ok` 仅作临时兼容。
...
- 8794五态协议和视觉方向校准均已部署到8794与8793；当前用真实题图评审候选排序与五态归因，再决定是否提升8790。
...
2. 真实评审通过后，再独立决定是否把五态与方向校准一起提升到8790。
3. 提升完成后定义计划/权限协议并运行影子规划，只记录计划与审核结果，不执行AI计划。
