...
- 2026-07-11 扩展并发实验：`rerank_candidates_concurrent(...)` 支持 `max_workers`、单候选 HTTP 超时和每候选耗时回调；默认搜索仍未使用这些实验参数。字母库 `5位移法/题目/7.jpg` 的前 10 个钢架候选，在 `max_workers=10` 下全部并发成功；同轮串行 26.48s，并发 4.01s，提速 6.603x，Top 路径一致。模型耗时存在明显长尾：不同轮最慢单候选分别为 10.721s、2.829s，不能把某张题图固定视为慢图。
- 显式启用单候选超时时，SDK 必须关闭自动重试，否则 1s 请求会因多次重试拖到约 11s。当前实验实现会把超时项保留为 `rerank_status=timeout`，保留粗筛分且不赋视觉 0 分；这避免潜在正确答案被静默丢弃，但结果是“待复筛”而非可信最终排序。生产 UI/飞书/Agent 在具备待复筛展示或异步补结果前，不应把硬超时作为默认策略。
- 复筛输出规则：
