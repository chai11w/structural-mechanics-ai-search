...
- 每次搜题模型费用台账已在独立开发分支完成，尚未部署到8790；当前8790服务和生产状态未改动。
...
- 费用台账待确定提醒阈值与用户可见范围，再经8793验证后提升到8790。
...
1. 根据22候选峰值实测确定单次费用提醒阈值、日报规则和后台展示范围。
2. 在8793验证费用记录不影响正常搜题后，将费用台账提升到8790；仍不改现有服务直到明确部署。
3. 费用底座稳定后，回到8794定义并记录影子计划与权限审核协议。
