...
- 8794已实现只记录不执行的影子规划与权限审核；旧7阶段×15话术矩阵直接调用Planner，仅保留为结构压力测试。新增35条金标准真实入口评估，从`Agent.handle_text()`经过安全/固定业务/Planner准入门，并对同一意图结果成对比较开关影子规划。三轮真实千问共105次中91次符合金标准，固定明确路径误触发Planner=0、回答/状态/原工具调用差异=0；10条为有动作影子计划、16条为空计划、3条权限拒绝，14次命中禁止语义动作且`needs_confirmation`仍为0，暂不进入有限执行。
...
- Stage 5 正在修正模糊业务的语义依据与确认边界：保留原话，区分明确关键词与推测关键词；题号、候选编号、章节和全局搜索授权缺乏原话证据时必须进入`needs_confirmation`，不能生成可执行动作。
...
1. 为模糊补充结果增加明确/推测关键词、原话证据、置信度和`requires_confirmation`，先锁定“就这样吧/哪个靠谱/别的那个”等禁止自动选择样本。
2. 重跑35条金标准三轮真实入口评估，要求固定路径误触发、可见差异和禁止动作均为0；空计划继续单独统计。
3. 通过后再讨论只读`inspect_question`能力和8793真实评审；有限执行与LangGraph继续后置。
