...
- 8794源码与运行隔离、安全回答V0判定、回答契约、离线生成器、千问评测、Agent接线及候选专用启动参数已完成；当前8794仍是未开启参数的旧进程，待受控重启后才会加载并启用。
...
1. 记录当前8794旧进程的业务/混合HTTP基线和四个服务健康状态，再受控重启仅8794并开启安全回答参数。
2. 用真实“你是谁”、纯业务和混合请求验收回答、业务等价、会话状态与其他服务健康。
3. 8794验收通过后再讨论替换8790；随后统一工具结果为五态协议，并定义计划/权限协议运行影子规划。
