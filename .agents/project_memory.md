...
- Intent V2 已完成方案审查、`ActionDecisionV2` 协议、状态—动作权限矩阵、旁路 `ConversationContextV2`、首批 12 组已批准困难种子和 28 组平衡扩展草案；两组可组合为 40 条逻辑评测集。离线评测器已支持多片段加载及成对报告比较，当前均为旁路实现，没有改动 Prompt、`agent.py`、V1 状态机或检索链路。
...
- `ConversationContextV2` 尚未接入 `AgentState` 运行入口或 SQLite 持久化；尚未实现 Intent V2 推理、歧义追问执行、专业交流回复、一次性 `pending_chapter` 持久化/消费与 V1/V2 影子对照。
- 28 条扩展样本仍是 `review_draft`，需先审查金标准，不能把组合后的 40 条直接称为正式评测集；V2 推理尚未实现，因此尚无真实 V1/V2 成对指标。
...
2. 在已完成的旁路 `ConversationContextV2` 上实现受约束 Intent V2，并在同一 40 条样本上同时运行 `V1-full` / `V2-full`，输出逐条改善、退化和指标差值；不得只报告单边准确率。
3. 成对基线完成后再考虑影子接入；LangGraph 留到动作协议稳定后再评估。
