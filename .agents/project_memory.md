...
2. 冻结开发回归集后补充未参与实现的盲测改写集，复跑同配置 `V1-full` / `V2-full` 并验证模型稳定性；盲测同时检查交流回复单行、简洁、领域受限和零业务状态副作用，不能只用开发集 100% 宣称成熟。
3. 盲测达标后再讨论 Intent V2 影子接入和 SQLite 新字段；LangGraph 留到动作协议稳定后再评估。
