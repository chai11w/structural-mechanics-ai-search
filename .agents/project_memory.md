...
- V1 展示目录 `F:\cc\7-题库检索` 保持在 `codex/llm-rerank-support-filter`；V2 独立工作区 `F:\cc\7-题库检索-intent-v2` 使用 `codex/intent-v2`，不得交叉运行或提交文件。V1 的 `data/feishu_chapter_failure_log.jsonl` 用户日志改动继续保留。
...
- Intent V2 已完成方案审查、`ActionDecisionV2` 协议、状态—动作权限矩阵、旁路 `ConversationContextV2`、受约束 `intent_v2.py` 和简洁专业交流外壳 `reply_shell_v2.py`；首批 12 组已批准困难种子与 28 组平衡扩展草案可组合为 40 条逻辑评测集。离线评测器已支持 V1/V2 规则层与 full 成对报告，当前均为旁路实现，没有改动 V1 Prompt、`agent.py`、V1 状态机或检索链路。
- V1 纯规则路径在 12 条困难种子上的冻结结果为：严格协议 0/12、动作类型 3/12、安全成功 0/12、本应安全响应却执行题库动作 4 次。该集合刻意集中上下文和安全失败，不代表 V1 日常请求总体准确率；正式指标需扩充约 40 条、纳入普通样本后再计算。
- 2026-07-14 在组合 40 条开发回归集上的同配置单次对照：V1 规则层严格 12/40（30%），V2 规则层 37/40（92.5%）；`V1-full` 严格 22/40（55%）、动作 70%、安全成功 57.5%、危险误执行 3 次，`V2-full` 严格/动作/安全成功均为 40/40（100%）、危险误执行 0 次，逐条改善 18、退化 0。两边使用同一 Qwen 默认模型、temperature=0 和相同样本；V2 仅 3 条进入上下文模型。

## Not Implemented

- 尚未引入 LangGraph 图或 checkpoint，也未创建、连接新的飞书机器人。
- `ConversationContextV2`、Intent V2 和交流外壳尚未接入 `AgentState` 运行入口或 SQLite 持久化；尚未实现一次性 `pending_chapter` 实际消费与 V1/V2 影子对照。
- 28 条扩展样本仍是 `review_draft`，需先审查金标准，不能把组合后的 40 条直接称为正式盲测集；当前真实 V1/V2 成对指标只用于开发回归判断。
- 40 条样本已经参与 Intent V2 规则和上下文实现，属于开发回归集，100% 不能外推为新表达的泛化准确率；冻结标签后还需要未参与开发的盲测/改写集，并至少复跑一次模型稳定性。
- 首次 20 条盲测已在运行前以提交 `97a6abb` 冻结。首轮 `V1-full` 严格 11/20（55%），`V2-full` 16/20（80%），改善 7、退化 2；V2 有 2 次危险误执行（多候选“换一个”被擅自选择、删除委婉表达“从库里清掉”被误当成选候选），因此发布门槛失败，结果详见 `.agents/intent_v2_blind_01_result.md`。
- Blind 01 的两个安全缺口已用代码证据门修复：破坏性动词与题库对象组合优先拒绝；模型推断的题号/候选编号必须能由明确编号、唯一剩余候选、上一题或唯一未完成题验证。原两条危险样本真实 Qwen 专项复测 2/2、危险执行 0；本阶段未顺手修复另外两个普通准确率问题。
- 随后用通用明确表达语法修复“第 2 小题 + 章节覆盖”和“下一张发的是某章节”的目标判断；Blind 01 完整回归为 V2 19/20（95%）、危险执行 0、相对 V1 改善 9/退化 1。唯一剩余失败“答案候选里我要排第三的”被安全追问，当前故意不加短语专用规则追求 20/20。
...
1. 审查并批准 28 条扩展草案，使 40 条样本的上下文、标准动作和安全预期成为冻结金标准。
2. 建立 Blind 02 验证新表达泛化，并加入多种“候选语义 + 数字排名”改写；只有证明是系统性缺口，才考虑通用数字证据规则，不为单条样本加关键词。
3. Blind 01 此后只作为回归集。Blind 02 达标前不接入 Agent/SQLite；LangGraph 留到动作协议稳定后再评估。
