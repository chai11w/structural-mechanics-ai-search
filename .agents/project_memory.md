...
- 8790/8794同mock HTTP行为、状态与调用序列已有基线测试；8794独立工作区全量测试233项、主线/观测镜像一致性19项通过，8788/8790/8793/8794接口均实测可用。
...
- 8794源码与运行隔离、安全回答V0判定、默认关闭的固定回复接线、回答契约、离线生成器、千问离线评测及Agent可选生成接线已完成；8794启动脚本尚未注入生成器且运行实例未开启，因此没有改变现有回答、状态和工具行为。
...
1. 讨论为8794候选启动入口增加默认关闭的安全回答参数，并在开启时注入千问生成器；8790/8793/8788入口不改。
2. 先验证关闭参数的HTTP行为等价，再受控重启仅8794开启参数，用真实“你是谁”、业务和混合请求验收。
3. 8794验收后统一工具结果为五态协议，并定义计划/权限协议运行影子规划。
