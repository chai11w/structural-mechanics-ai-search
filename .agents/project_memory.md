...
- V2 主线在独立工作区 `F:\cc\7-题库检索-intent-v2`，线上 8790 从该工作区隐藏启动；`data/feishu_chapter_failure_log.jsonl` 用户日志改动继续保留并禁止提交。
...
- Demo 的临时气泡由真实 Agent 阶段事件驱动：自动识别章节、用户补充章节后进入普通搜索时显示对应章节，全局搜索显示较慢提示；完成、失败或追问时都会结束动态状态。
...
- 隔离运行时已覆盖未经邀请拒绝、授权后单/多/无结果、无结果后指定章节、按 `candidate_rank` 取答案和跨重启恢复；V1 退役后当前全量 175 项测试通过。真实题库生产工具单题冒烟返回1个严格候选，证明链路可运行但不能外推准确率。
