...
- 评测器已区分直接成功、安全追问、可恢复结果和错误执行。Blind 01/02 分别为95%/68.75%直接成功，两套均100%可恢复、错误执行0；按90%/10%假设的直接成功率为92.375%，流量比例尚未实证。
...
1. 建立“追问后用户补充一次”的多轮恢复集，测最终完成率和额外回合数，不把安全追问等同于任务结束。
2. 冻结 Blind 03 验证窄安全门与未来章节语法的泛化，不针对剩余长尾失败加词表。
3. 多轮恢复和新盲测达标后再讨论 Agent/SQLite 接入；LangGraph仍留到动作协议稳定后评估。
