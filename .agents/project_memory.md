...
- Intent V2 已完成动作协议、权限矩阵、净化上下文、受约束推理链、交流外壳、成对评测器和显式版本适配。V2 使用独立 `.tmp_tiku_agent_v2`，默认V1及检索链路未改；40条开发集结果只用于回归。
...
1. 在独立8791入口做人工试运行，重点验证追问后补充、上一题、剩余题和跨重启待用章节；不停止8790。
2. 冻结 Blind 03 验证运行时接入后的泛化，不针对剩余长尾失败加词表。
3. 试运行无错误执行后再讨论默认切换；LangGraph仍留到确有编排需求时评估。
