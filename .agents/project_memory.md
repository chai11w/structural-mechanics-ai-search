...
- Blind 01 修复后 V2 为 19/20、危险执行 0；新建 Blind 02 首次同场结果反而低于 V1（V1 10/16，V2 9/16），且 V2 有 2 次危险执行。当前重点是通用语义冲突证据门，不接入稳定入口；详见两份 blind result 文档。
...
- Blind 02 暴露的两类通用安全缺口尚未修复：禁止保留/移除语义会被候选数字覆盖；问候加模糊任务延续会被误判为业务动作。其余安全追问不为追求满分而加短语专用规则。
...
1. 讨论并设计通用“正向动作证据 + 语义冲突否决”校验，不按 Blind 02 的失败句子追加关键词。
2. 新安全门先用专项变体集验证，再建立未参与修复的 Blind 03。
3. 新盲测达标前不接入 Agent/SQLite；LangGraph 留到动作协议稳定后再评估。
