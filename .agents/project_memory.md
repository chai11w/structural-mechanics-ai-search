...
- 8790/8794同mock HTTP行为、状态与调用序列已有基线测试；8794独立工作区全量测试225项、主线/观测镜像一致性19项通过，8788/8790/8793/8794接口均实测可用。
...
- 8794源码与运行隔离、安全回答V0判定、默认关闭的固定回复接线、回答契约及离线生成器骨架已经完成；尚未连接真实模型，运行中的8794未开启该开关，因此没有改变现有回答、状态和工具行为。
...
1. 讨论千问离线适配器和评测输出格式；先选10条核心问题各采样3次，不接入Agent或8794。
2. 根据30次小样本的契约通过率、回答质量和延迟决定是否调整提示词，再运行38条完整离线验收。
