...
2. 先讨论并修复 Blind 01 暴露的两个安全缺口：删除委婉表达必须在代码层拒绝；存在多个可替代候选时，“换一个”必须由代码强制追问，不能信任模型猜排名。
3. 再处理“第 2 小题 + 章节覆盖”和“下一张发的是某章节”的明确表达覆盖；Blind 01 修复后只作为回归集，另建 Blind 02 验证泛化。盲测达标前不接入 Agent/SQLite；LangGraph 留到动作协议稳定后再评估。
