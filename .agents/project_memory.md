...
- Intent V2 已完成方案审查、`ActionDecisionV2` 协议、状态—动作权限矩阵、首批 12 组金标准困难种子和离线评测器；当前均为旁路实现，没有改动 Prompt、`agent.py`、V1 状态机或检索链路。
...
- 尚未将 12 条困难种子扩充为约 40 组平衡的真实多轮意图评测集；当前基线只能衡量已知难点，不能外推总体准确率。
...
1. 将已批准的 12 条困难种子扩充为约 40 条平衡评测集，补入 V1 应能稳定通过的普通明确表达，并继续覆盖专业交流外壳。
2. 用同一离线评测器运行正式 V1 规则基线，分别报告严格协议、动作类型、安全成功、危险执行和分类指标。
3. 正式基线完成后实现 ConversationContext V2、受约束 Qwen 动作判断和歧义追问；LangGraph 留到动作协议稳定后再评估。
