...
- 8790/8794同mock HTTP行为、状态与调用序列已有基线测试；8794独立工作区全量测试229项、主线/观测镜像一致性19项通过，8788/8790/8793/8794接口均实测可用。
...
- 小样本角色与回答风格已获用户认可；固定回复只作为模型异常或真正越界时的兜底，不要求模型逐字复现模板，也不为制造变化而提高随机性。尚需完成38条纯对话的完整离线验收，千问仍未接入Agent。
...
1. 讨论38条纯对话完整离线验收的采样次数与人工审查方式；仍不接入Agent或8794。
2. 完整验收通过后，为Agent增加默认关闭的模型生成接线并验证固定兜底、零工具和零业务状态变化，再讨论是否在隔离8794启用。
