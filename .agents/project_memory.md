...
- Intent V2 已完成方案审查、`ActionDecisionV2` 协议、状态—动作权限矩阵、首批 12 组已批准困难种子和 28 组平衡扩展草案；两组可组合为 40 条逻辑评测集。离线评测器已支持多片段加载及成对报告比较，当前均为旁路实现，没有改动 Prompt、`agent.py`、V1 状态机或检索链路。
...
2. 实现 ConversationContext V2 与旁路 Intent V2 后，在同一 40 条样本上同时运行 `V1-full` / `V2-full`，输出逐条改善、退化和指标差值；不得只报告单边准确率。
