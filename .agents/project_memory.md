...
- `tiku_agent/tools.py`：多题工具分为布局/题目列表识别与隔离裁图准备两步，均已注册到 Agent 编排；单题不调用裁图准备。
...
- Agent 处于多题 MVP 后的迭代阶段；待补图片附带文字的入口分流和真实多题图端到端回归。
...
- 自动图片入口会先调用多题布局识别；真实单题到候选实测约 14 秒，图片附带“按单题搜”等自然语言的快速分流入口尚未接入。
...
1. 增加图片附带自然语言的入口分流：用户明确要求单题时跳过多题布局识别，否则自动判断单题/多题。
2. 用独立真实多题图完成 Agent 端到端回归，覆盖裁图与无裁图回退。
