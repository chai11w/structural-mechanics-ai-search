...
- `tiku_agent/fastapi_demo.py` 与 `scripts/run_tiku_agent_demo.py`：独立 FastAPI 本地 demo 已实现，默认仅监听 `127.0.0.1:8790`，用 session cookie 连接持久会话；图片令牌避免暴露本地路径，识别期间有显式等待提示。Qwen 瞬时 HTTP 429/5xx/超时会自动重试；失败后可回复“重试”复用原图。
- FastAPI demo 使用桌面聊天布局：侧栏、新对话、居中消息流和底部输入框；上传题图会显示本地预览，`新对话` 会清除当前 session 和临时文件。
- `tiku_agent/tools.py`：首轮 scope 只判单题/多题；单题同时返回荷载/章节并直接检索，多题才调用详细题号/bbox/逐题荷载识别和裁图准备。重复同标注外荷载必须逐项保留；纯结构图即使被模型猜为第二章，也会因缺少可引用题干而降级为 `unknown` 并询问章节。
