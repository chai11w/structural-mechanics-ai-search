...
- 临时8792隔离网页/HTTP端到端验收通过：真实题图进入章节未知提示，明确同意后全局返回2个候选，按“第二个候选”取得1张答案图；未获邀请的全局搜索被追问拦截，发图前预设4力法后直接走普通章节检索，问候保持简洁。临时服务和页面均已关闭，8790未改动，本轮未运行V1对比。
...
2. 受控重试和隔离端到端验收均已通过；下一小步是把当前分支合回正式V2主线，并在部署前核对8790实际启动入口、运行目录和回退点。
3. 经用户确认后部署V2到8790并做线上冒烟；稳定后单独删除V1开关、代码和旧运行目录，普通章节检索、现有飞书和LangGraph边界保持不变。
