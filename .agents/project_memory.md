...
- A3-V1 已在 `8790` 生产：Qwen 整页理解 → GLM 框选/Pillow 裁图 → Qwen 双门禁 → 单题自动下行或多题选择 → A2/人工裁剪；8790、8896 和飞书储存入口默认关闭四方向 OCR，RapidOrientation 未接入。
...
- 当前 8790 为 refresh-recovery 固定 release，全仓 1222 项通过，运行、回退和 Trace 证据保持；公网已加载新 `demo.js` 且 `/health` 一致，但登录 `/api/session` 对账未闭环。用户继续实际试用；3.5.3 DONE/BLOCKED、3.5.4 DONE 的历史状态不可改写。
...
- refresh-recovery 已部署并由用户继续实际试用；已登录浏览器的 `/api/session` 可重复恢复验收仍未闭环，不声明该热修复 live acceptance 通过。
...
- RapidOrientation、Qwen 方向门禁和线上发布均未实现；2026-09 联合实验结论为 `keep_old`，不再接入。
...
- A3 裁剪固定为 GLM bbox + Pillow；方向预处理关闭，不恢复 Paddle 主链或默认四方向 OCR。
...
- 方向联合实验不支持上线：Qwen 精确角度 `41/56`、二值信号 `55/56`，RapidOrientation `49/56`；双门禁仍需阈值 `0.91` 才无危险动作，且只修正 `32/42` 个旋转样本，不能据此降低阈值。
...
1. 由用户继续实际试用，遇到刷新/恢复问题时保留可重复步骤；后续从已登录单标签页 `/api/session` 对账验收继续。
