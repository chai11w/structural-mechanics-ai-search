...
- Stage 5 语义授权闸门已完成，下一步待讨论Planner准入范围：是否让“明确但包含两个及以上动作的复杂请求”在固定业务仍照常处理的同时进入影子观察，从而得到真实可执行计划样本；暂不改变用户回复或业务执行。
...
1. 先讨论并定义复杂请求的影子准入：只考虑原话明确包含两个及以上合法动作的请求，固定业务响应和执行保持不变，Planner仍只记录。
2. 为该准入补金标准，要求存在能通过语义闸门的真实动作计划，同时单动作快速路径仍不调用Planner、回答/状态/工具差异仍为0。
3. 单独讨论两条固定业务误判是否修复；通过后再讨论关键只读工具拆分、`inspect_question`和8793真实评审。有限执行与LangGraph继续后置。
