...
- 严格全局搜索前两门已完成：协议增加无参数 `global_search`，权限要求 `WAIT_CHAPTER`、有效题图和已提供兜底选项；规则层可识别明确同意、拒绝、含糊回应和章节优先，模型输出仍由代码复核授权证据。尚未连接 Agent 或检索工具。
...
- 尚未实现跨章节粗筛聚合、去重、严格视觉门槛、结果渲染、持久状态和运行时工具；协议和意图识别通过不代表动作可在线执行。
...
1. 扩充跨章节检索离线样本；首轮7题实测所有 `score >= 0.999` 去重候选均复筛，按严格 `rerank_score > 0.95` 重算为自命中7/7、无结果0、精确文件比例87.5%、72次模型调用、平均7.62秒，无超时。只限制单批并发，不限制总候选或返回数。
2. 检索门通过后再接隔离运行时、状态持久化和文案；不改普通检索、现有飞书和 LangGraph 边界。
3. 接入后补无结果、多结果跨章节展示及用户按 `candidate_rank` 选择的集成测试，再决定是否上线。
