...
- 8790/8794同mock HTTP行为、状态与调用序列已有基线测试；8794独立工作区全量测试230项、主线/观测镜像一致性19项通过，8788/8790/8793/8794接口均实测可用。
...
- 8794源码与运行隔离、安全回答V0判定、默认关闭的固定回复接线、回答契约、离线生成器及千问离线评测已完成；千问仍未接入Agent，运行中的8794未开启该开关，因此没有改变现有回答、状态和工具行为。
...
- 尚未实现或接入安全自由回答生成；当前 `greeting / small_talk / capability_help` 等仍使用固定回复，且“你是谁”已作为真实错误样本进入8793评审。
...
1. 讨论为Agent增加默认关闭的模型生成接线：只替换已批准纯对话的固定回复，非合格消息继续原Intent V2。
2. 验证模型成功、异常和违规兜底均为零工具、零业务状态变化，且所有业务/混合路径完全等价后，再讨论是否在隔离8794启用。
