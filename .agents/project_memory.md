...
- 严格全局搜索第一门已完成：协议增加无参数 `global_search`，权限要求 `WAIT_CHAPTER`、有效题图和已提供兜底选项；未经邀请的跨章节盲搜仍为禁止请求。尚未连接 Prompt、Agent 或检索工具。
...
- 全局兜底新增12条评测：改造前 V1 直接成功3/12、可恢复11/12、错误执行1；当前 V2 直接成功5/12、可恢复12/12、错误执行0。下一门先提高授权同意识别，不接真实检索。

## Not Implemented

- 尚未引入 LangGraph 图或 checkpoint，也未创建、连接新的飞书机器人。
- 尚未把V2设为默认，也未连接新飞书机器人或建立真实流量影子统计；当前只允许在8791和独立运行目录试用。
- 尚未实现全局搜索 Prompt/规则识别、跨章节粗筛聚合、去重、严格视觉门槛、结果渲染、持久状态和运行时工具；协议存在不代表动作可在线执行。
...
1. 在新增12条样本上实现并对照 `global_search` 明确授权识别，保持未授权执行为0；先不连接检索工具。
2. 建立跨章节检索离线样本，验证 `score >= 0.999`、`rerank_score >= 0.95`、最多复筛6道/返回2道的精确率、无结果率、耗时和模型调用数。
3. 前两门通过后再接隔离运行时和文案；不改普通检索、现有飞书和 LangGraph 边界。
